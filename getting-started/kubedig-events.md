...
Note that KubeDig also alerts events blocked due to other system policy enforcement. For example, if an SELinux native rule blocks an action, KubeDig will report those as well as `DefaultPosture` events. Following is an example of such event:
