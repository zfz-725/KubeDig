...
For better understanding, you can check [the KubeDigHostPolicy spec diagram](../.gitbook/assets/kubedighostpolicy-spec-diagram.pdf).
...
* Common
  ...
  Make sure that you need to use `KubeDigHostPolicy`, not `KubeDigPolicy`.
...
* NodeSelector
  ...
  ```text
      ...
      kubernetes.io/hostname: [host name, (e.g., kubedig-dev)]
...
There is one options in each match.
  ...
  * fromSource
    If a path is specified in fromSource, kubedig will match only syscalls generated by the defined source. For better undrestanding, lets take the example below. Only unlink system calls generated by `/bin/bash` will be matched.
    ```text
      process:
        matchPaths:
        - path: /bin/sleep
