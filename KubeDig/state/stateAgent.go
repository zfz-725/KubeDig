...
// package state implements the state agent service which reports details about the node and containers being protected by KubeDig
package state
...
// StateAgent reports the state of containers/nodes protected by KubeDig
type StateAgent struct {
	...
	KubeDigNamespaces     map[string]types.Namespace
	KubeDigNamespacesLock *sync.RWMutex
}
...
func NewStateAgent(node *tp.Node, nodeLock *sync.RWMutex, containers map[string]tp.Container, containersLock *sync.RWMutex) *StateAgent {
	return &StateAgent{
		...
		KubeDigNamespaces:     make(map[string]tp.Namespace),
		KubeDigNamespacesLock: new(sync.RWMutex),
	}
}
...
func (sa *StateAgent) GetState(msg *emptypb.Empty, srv pb.StateAgent_GetStateServer) error {
	...
	sa.KubeDigNamespacesLock.RLock()
	for nsName, ns := range sa.KubeDigNamespaces {
	...
	sa.KubeDigNamespacesLock.RUnlock()
