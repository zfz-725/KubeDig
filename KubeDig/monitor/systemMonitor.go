...
func (mon *SystemMonitor) DestroyBPFMaps() {
	if mon.BpfNsVisibilityMap != nil {
		...
		if err != nil {
			mon.Logger.Warnf("error unpinning bpf map kubedig_visibility %v", err)
		}
		err = mon.BpfNsVisibilityMap.Close()
		if err != nil {
			mon.Logger.Warnf("error closing bpf map kubedig_visibility %v", err)
		}
	}
	...
	if mon.BpfConfigMap != nil {
		...
		if err != nil {
			mon.Logger.Warnf("error unpinning bpf map kubedig_config %v", err)
		}
		err = mon.BpfConfigMap.Close()
		if err != nil {
			mon.Logger.Warnf("error closing bpf map kubedig_config %v", err)
		}
	}
}
