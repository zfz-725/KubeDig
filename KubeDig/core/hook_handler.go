...
const kubedigDir = "/var/run/kubedig"

// ListenToHook starts listening on a UNIX socket and waits for container hooks
// to pass new containers
func (dm *KubeDigDaemon) ListenToHook() {
	dm.Logger.Print("Started to monitor OCI Hook events")
	if err := os.MkdirAll(kubedigDir, 0750); err != nil {
		log.Fatal(err)
	}

	listenPath := filepath.Join(kubedigDir, "ka.sock")
	err := os.Remove(listenPath) // in case kubedig crashed and the socket wasn't removed
...
func (dm *KubeDigDaemon) handleConn(conn net.Conn, ready *atomic.Bool) {
	// We need to makes sure that no containers accepted until all containers created before KubeDig
	...
	// to send that these containers are created. Which will leave KubeDig in an incorrect state.
	defer conn.Close()
	buf := make([]byte, 4096)
	...
	for {
		...
		if data.Detached {
			// we want KubeDig to start accepting containers after
...
func (dm *KubeDigDaemon) handleContainerCreate(container types.Container) {
	...
	if dm.SystemMonitor != nil && cfg.GlobalCfg.Policy {
		...
		if len(endpoint.SecurityPolicies) > 0 { // struct can be empty or no policies registered for the endpoint yet
			...
			if dm.RuntimeEnforcer != nil && endpoint.PolicyEnabled == types.KubeDigPolicyEnabled {
...
func (dm *KubeDigDaemon) handleContainerDelete(containerID string) {
	dm.ContainersLock.Lock()
