...
func (dm *KubeDigDaemon) SetContainerNSVisibility() {
...
func (dm *KubeDigDaemon) WatchConfigChanges() {
...
func (dm *KubeDigDaemon) MatchandUpdateContainerSecurityPolicies(cid string) {
	...
	for idx, ep := range dm.EndPoints {
		...
		if ep.EndPointName == dm.Containers[cid].ContainerName || kl.MatchIdentities(ep.Identities, containerIdentities) {
			...
			if cfg.GlobalCfg.Policy {
				...
				if ep.PolicyEnabled == tp.KubeDigPolicyEnabled {
...
func (dm *KubeDigDaemon) MatchandRemoveContainerFromEndpoint(cid string) {
	container := dm.Containers[cid]
	for idx, ep := range dm.EndPoints {
		_, containerIdentities := kl.GetLabelsFromString(container.Labels)
...
func (dm *KubeDigDaemon) handlePolicyEvent(eventType string, createEndPoint bool, secPolicy tp.SecurityPolicy, newPoint tp.EndPoint, endpointIdx int, containername string) (int, pb.PolicyStatus) {
	if containername == "" {
		containername = newPoint.ContainerName
	}

	appArmorAnnotations := map[string]string{}
	appArmorAnnotations[containername] = "kubedig_" + containername
	...
	if eventType == "ADDED" {
		...
		if createEndPoint {
			...
			newPoint.AppArmorProfiles = []string{"kubedig_" + containername}
...
func (dm *KubeDigDaemon) ParseAndUpdateContainerSecurityPolicy(event tp.K8sKubeDigPolicyEvent) pb.PolicyStatus {
	...
	if _, ok := secPolicy.Spec.Selector.MatchLabels["kubedig.io/container.name"]; ok && len(secPolicy.Spec.Selector.MatchLabels) > 1 {
		dm.Logger.Warnf("Failed to apply policy. Cannot use \"kubedig.io/container.name\" and other labels together.")
		return pb.PolicyStatus_Invalid
		...
		dm.Logger.Warnf("Received policy for AppArmor enforcer without \"kubedig.io/container.name\"")
		return pb.PolicyStatus_Invalid
	}
	...
	for k, v := range secPolicy.Spec.Selector.MatchLabels {
		...
		if k == "kubedig.io/container.name" {
			...
			if err != nil {
				dm.Logger.Warnf("Failed to parse expression for \"kubedig.io/container.name\": %s", err.Error())
				return pb.PolicyStatus_Invalid
			}
	...
	if !dm.K8sEnabled && (cfg.GlobalCfg.KVMAgent || cfg.GlobalCfg.Policy) {
		if event.Type == "ADDED" || event.Type == "MODIFIED" {
			...
			dm.backupKubeDigContainerPolicy(secPolicy)
		} else if event.Type == "DELETED" {
...
// backupKubeDigHostPolicy Function
func (dm *KubeDigDaemon) backupKubeDigHostPolicy(policy tp.HostSecurityPolicy) {
	// Check for "/opt/kubedig/policies" path. If dir not found, create the same
...
// Back up KubeDig container policies in /opt/kubedig/policies
func (dm *KubeDigDaemon) backupKubeDigContainerPolicy(policy tp.SecurityPolicy) {
	// Check for "/opt/kubedig/policies" path. If dir not found, create the same
	if _, err := os.Stat(cfg.PolicyDir); err != nil {
		if err = os.MkdirAll(cfg.PolicyDir, 0700); err != nil {
			kg.Warnf("Dir creation failed for [%v]", cfg.PolicyDir)
...
func (dm *KubeDigDaemon) restoreKubeDigPolicies() {
	if _, err := os.Stat(cfg.PolicyDir); err != nil {
		kg.Warn("Policies dir not found for restoration")
		return
	}

	// List all policies files from "/opt/kubedig/policies" path
		...
		for _, file := range policyFiles {
			if data, err := os.ReadFile(cfg.PolicyDir + file.Name()); err == nil {
				...
				if _, ok := k.Metadata["namespaceName"]; ok { // ContainerPolicy contains namespaceName
					var containerPolicy tp.K8sKubeDigPolicy
					if err := json.Unmarshal(data, &containerPolicy); err == nil {
						containerPolicy.Metadata.Name = k.Metadata["policyName"]
						dm.ParseAndUpdateContainerSecurityPolicy(tp.K8sKubeDigPolicyEvent{
							Type:   "ADDED",
					...
					var hostPolicy tp.K8sKubeDigHostPolicy
					if err := json.Unmarshal(data, &hostPolicy); err == nil {
						hostPolicy.Metadata.Name = k.Metadata["policyName"]
						dm.ParseAndUpdateHostSecurityPolicy(tp.K8sKubeDigHostPolicyEvent{
							Type:   "ADDED",
							Object: hostPolicy,
						})
					} else {
...
func (dm *KubeDigDaemon) removeBackUpPolicy(name string) {
