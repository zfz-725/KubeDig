...
// Package core is responsible for initiating and maintaining interactions between external entities like K8s,CRIs and internal KubeDig entities like eBPF Monitor and Log Feeders
package core
...
import (
	...
	"github.com/zfz-725/KubeDig/KubeDig/common"
	...
	"github.com/zfz-725/KubeDig/KubeDig/policy"
	"github.com/zfz-725/KubeDig/KubeDig/presets"
	"github.com/zfz-725/KubeDig/KubeDig/state"
	...
	efc "github.com/zfz-725/KubeDig/KubeDig/enforcer"
	fd "github.com/zfz-725/KubeDig/KubeDig/feeder"
	kvm "github.com/zfz-725/KubeDig/KubeDig/kvmAgent"
...
// KubeDigDaemon Structure
type KubeDigDaemon struct {
	// node
	Node     tp.Node
...
// NewKubeDigDaemon Function
func NewKubeDigDaemon() *KubeDigDaemon {
	dm := new(KubeDigDaemon)
...
// DestroyKubeDigDaemon Function
func (dm *KubeDigDaemon) DestroyKubeDigDaemon() {
	close(StopChan)

	if dm.RuntimeEnforcer != nil {
		// close runtime enforcer
		if dm.CloseRuntimeEnforcer() {
			dm.Logger.Print("Stopped KubeDig Enforcer")
		}
	}
	...
	if dm.SystemMonitor != nil {
		...
		if dm.CloseSystemMonitor() {
			dm.Logger.Print("Stopped KubeDig Monitor")
		}
	}
	...
	if dm.Logger != nil {
		dm.Logger.Print("Terminated KubeDig")
	} else {
		kg.Print("Terminated KubeDig")
	}
	...
	if dm.Logger != nil {
		// close logger
		if dm.CloseLogger() {
			kg.Print("Stopped KubeDig Logger")
		}
	}
...
func (dm *KubeDigDaemon) CloseLogger() bool {
	if err := dm.Logger.DestroyFeeder(); err != nil {
		kg.Errf("Failed to destroy KubeDig Logger (%s)", err.Error())
		return false
	}
	return true
}
...
func (dm *KubeDigDaemon) InitSystemMonitor() bool {
...
func (dm *KubeDigDaemon) MonitorSystemEvents() {
	dm.WgDaemon.Add(1)
	defer dm.WgDaemon.Done()
...
func (dm *KubeDigDaemon) CloseSystemMonitor() bool {
	if err := dm.SystemMonitor.DestroySystemMonitor(); err != nil {
		dm.Logger.Errf("Failed to destroy KubeDig Monitor (%s)", err.Error())
		return false
	}
	return true
}
...
func (dm *KubeDigDaemon) InitRuntimeEnforcer(pinpath string) bool {
...
func (dm *KubeDigDaemon) CloseRuntimeEnforcer() bool {
	if err := dm.RuntimeEnforcer.DestroyRuntimeEnforcer(); err != nil {
		dm.Logger.Errf("Failed to destory KubeDig Enforcer (%s)", err.Error())
		return false
	}
	return true
}
...
func (dm *KubeDigDaemon) InitPresets(logger *fd.Feeder, monitor *mon.SystemMonitor) bool {
	dm.Presets = presets.NewPreset(dm.Logger, dm.SystemMonitor)
	return dm.Presets != nil
}

// ClosePresets Function
func (dm *KubeDigDaemon) ClosePresets() bool {
...
func (dm *KubeDigDaemon) InitStateAgent() bool {
...
func (dm *KubeDigDaemon) CloseStateAgent() bool {
...
// == Health Server == //
// =================== //
func (dm *KubeDigDaemon) SetHealthStatus(serviceName string, healthStatus grpc_health_v1.HealthCheckResponse_ServingStatus) bool {
...
func KubeDig() {
	// create a daemon
	dm := NewKubeDigDaemon()
	// Enable KubeDigHostPolicy for both VM and KVMAgent and in non-k8s env
	if cfg.GlobalCfg.KVMAgent || (!cfg.GlobalCfg.K8sEnv && cfg.GlobalCfg.HostPolicy) {
		...
		dm.Node.Labels["kubedig.io/hostname"] = dm.Node.NodeName

		dm.Node.Identities = append(dm.Node.Identities, "kubedig.io/hostname"+"="+dm.Node.NodeName)
		...
		for timeout := 0; timeout <= 60; timeout++ {
			...
			if nodeIP == "" && timeout == 60 {
				kg.Print("The node information is not available, terminating KubeDig")
	...
	if !dm.InitLogger() {
		kg.Err("Failed to initialize KubeDig Logger")
	...
	dm.Logger.Print("Initialized KubeDig Logger")
	...
	if cfg.GlobalCfg.Policy || cfg.GlobalCfg.HostPolicy {
		...
		if !dm.InitSystemMonitor() {
			dm.Logger.Err("Failed to initialize KubeDig Monitor")
		...
		dm.Logger.Print("Initialized KubeDig Monitor")
		...
		if !dm.InitRuntimeEnforcer(dm.SystemMonitor.PinPath) {
			dm.Logger.Print("Disabled KubeDig Enforcer since No LSM is enabled")
		} else {
			dm.Logger.Print("Initialized KubeDig Enforcer")
	...
	if dm.K8sEnabled && cfg.GlobalCfg.Policy {
		...
		if cfg.GlobalCfg.UseOCIHooks &&
			...
			// monitor NRI events
			go dm.MonitorNRIEvents()
		} else if cfg.GlobalCfg.CRISocket != "" { // check if the CRI socket set while executing kubedig exists
	...
	if !dm.K8sEnabled && (enableContainerPolicy || cfg.GlobalCfg.HostPolicy) {
		...
		if cfg.GlobalCfg.HostPolicy {
			...
			dm.Node.PolicyEnabled = tp.KubeDigPolicyEnabled
		...
		//Enable grpc service to send kubedig data to client in unorchestrated mode
		probe := &Probe{}
	...
	// == //
	go dm.SetKdigData()
	dm.Logger.Print("Initialized KubeDig")
	// == //

	if cfg.GlobalCfg.KVMAgent || !dm.K8sEnabled {
		// Restore and apply all kubedig host security policies
		dm.restoreKubeDigPolicies()
	}
	// == //
	...
	if !cfg.GlobalCfg.CoverageTest {
		...
		<-sigChan
		dm.Logger.Print("Got a signal to terminate KubeDig")
	}
...
func (dm *KubeDigDaemon) checkNRIAvailability() bool {
