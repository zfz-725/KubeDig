...
func (ch *ContainerdHandler) GetContainerInfo(ctx context.Context, containerID string, eventpid uint32, OwnerInfo map[string]tp.PodOwner) (tp.Container, error) {
	...
	} else if val, ok := containerLabels["kubedig.io/namespace"]; ok {
		container.NamespaceName = val
	} else {
	...
	if !cfg.GlobalCfg.K8sEnv {
		...
		if _, ok := containerLabels["kubedig.io/container.name"]; !ok {
			labels = append(labels, "kubedig.io/container.name="+container.ContainerName)
		}
...
func (dm *KubeDigDaemon) UpdateContainerdContainer(ctx context.Context, containerID string, containerPid uint32, action string) bool {
	...
	if action == "start" {
		...
		if _, ok := dm.Containers[container.ContainerID]; !ok {
			...
			if !dm.K8sEnabled {
				...
				case "ADDED":
					...
					endPoint.PolicyEnabled = tp.KubeDigPolicyEnabled
					...
					endPoint.AppArmorProfiles = []string{"kubedig_" + container.ContainerName}
		...
		if dm.SystemMonitor != nil && cfg.GlobalCfg.Policy {
			...
			if len(endPoint.SecurityPolicies) > 0 { // struct can be empty or no policies registered for the endPoint yet
				...
				if dm.RuntimeEnforcer != nil && endPoint.PolicyEnabled == tp.KubeDigPolicyEnabled {
					// enforce security policies
					dm.RuntimeEnforcer.UpdateSecurityPolicies(endPoint)
				}
				if dm.Presets != nil && endPoint.PolicyEnabled == tp.KubeDigPolicyEnabled {
...
func (dm *KubeDigDaemon) MonitorContainerdEvents() {
	dm.WgDaemon.Add(1)
...
func (dm *KubeDigDaemon) handleContainerdEvent(envelope *events.Envelope, context context.Context) {
	if envelope == nil {
		return
	}
