...
import (
	...
	"github.com/zfz-725/KubeDig/KubeDig/monitor"
	tp "github.com/zfz-725/KubeDig/KubeDig/types"
	ksp "github.com/zfz-725/KubeDig/pkg/KubeDigController/api/security.kubedig.com/v1"
	kspinformer "github.com/zfz-725/KubeDig/pkg/KubeDigController/client/informers/externalversions"
...
const (
	KubeDigPolicy        string = "KubeDigPolicy"
	KubeDigClusterPolicy string = "KubeDigClusterPolicy"
...
func (dm *KubeDigDaemon) HandleNodeAnnotations(node *tp.Node) {
	if _, ok := node.Annotations["kubedig-policy"]; !ok {
		node.Annotations["kubedig-policy"] = "enabled"
	}

	if node.Annotations["kubedig-policy"] != "enabled" && node.Annotations["kubedig-policy"] != "disabled" && node.Annotations["kubedig-policy"] != "audited" {
		node.Annotations["kubedig-policy"] = "enabled"
	}
	...
	if v, ok := node.Labels["kubedig.io/enforcer"]; ok {
		lsm = v
	...
	if !hasBPF && !hasSelinux && !hasAppArmor {
		...
		if node.Annotations["kubedig-policy"] == "enabled" {
			node.Annotations["kubedig-policy"] = "audited"
		}
	}

	if kl.IsInK8sCluster() && hasSelinux {
		// exception: KubeDig in a daemonset even though SELinux is enabled
		if node.Annotations["kubedig-policy"] == "enabled" {
			node.Annotations["kubedig-policy"] = "audited"
		}
	}
	...
	if node.Annotations["kubedig-policy"] == "enabled" {
		node.PolicyEnabled = tp.KubeDigPolicyEnabled
	} else if node.Annotations["kubedig-policy"] == "audited" {
		node.PolicyEnabled = tp.KubeDigPolicyAudited
	} else { // disabled
		node.PolicyEnabled = tp.KubeDigPolicyDisabled
	}

	if _, ok := node.Annotations["kubedig-visibility"]; !ok {
		node.Annotations["kubedig-visibility"] = cfg.GlobalCfg.HostVisibility
	}

	for _, visibility := range strings.Split(node.Annotations["kubedig-visibility"], ",") {
...
func (dm *KubeDigDaemon) checkAndUpdateNode(item *corev1.Node) {
	node := tp.Node{}
...
func (dm *KubeDigDaemon) WatchK8sNodes() {
	kg.Printf("GlobalCfg.Host=%s, KUBEDIG_NODENAME=%s", cfg.GlobalCfg.Host, os.Getenv("KUBEDIG_NODENAME"))

	nodeName := os.Getenv("KUBEDIG_NODENAME")
	if nodeName == "" {
...
func (dm *KubeDigDaemon) UpdateEndPointWithPod(action string, pod tp.K8sPod) {
	if action == addEvent {
		...
		// update policy flag
		if pod.Annotations["kubedig-policy"] == "enabled" {
			newPoint.PolicyEnabled = tp.KubeDigPolicyEnabled
		} else if pod.Annotations["kubedig-policy"] == "audited" {
			newPoint.PolicyEnabled = tp.KubeDigPolicyAudited
		} else { // disabled
			newPoint.PolicyEnabled = tp.KubeDigPolicyDisabled
		}

		// parse annotations and update visibility flags
		for _, visibility := range strings.Split(pod.Annotations["kubedig-visibility"], ",") {
			if visibility == "process" {
		...
		if cfg.GlobalCfg.Policy {
			...
			for _, endpoint := range endpoints {
				...
				if newPoint.PolicyEnabled == tp.KubeDigPolicyEnabled {
		...
		if len(endpoints) == 0 {
			...
			// update policy flag
			if pod.Annotations["kubedig-policy"] == "enabled" {
				newEndPoint.PolicyEnabled = tp.KubeDigPolicyEnabled
			} else if pod.Annotations["kubedig-policy"] == "audited" {
				newEndPoint.PolicyEnabled = tp.KubeDigPolicyAudited
			} else { // disabled
				newEndPoint.PolicyEnabled = tp.KubeDigPolicyDisabled
			}
			...
			for _, endpoint := range endpoints {
				if cfg.GlobalCfg.Policy {
					...
					if endpoint.PolicyEnabled == tp.KubeDigPolicyEnabled {
						// enforce security policies
						if !kl.ContainsElement(dm.SystemMonitor.UntrackedNamespaces, endpoint.NamespaceName) {
							if dm.RuntimeEnforcer != nil {
								dm.RuntimeEnforcer.UpdateSecurityPolicies(endpoint)
...
func (dm *KubeDigDaemon) HandleUnknownNamespaceNsMap(container *tp.Container) {
...
func (dm *KubeDigDaemon) handlePodEvent(event string, obj *corev1.Pod) {
	...
	if _, ok := pod.Annotations["kubedig-policy"]; !ok {
		pod.Annotations["kubedig-policy"] = "enabled"
	}

	if pod.Annotations["kubedig-policy"] != "enabled" && pod.Annotations["kubedig-policy"] != "disabled" && pod.Annotations["kubedig-policy"] != "audited" {
		pod.Annotations["kubedig-policy"] = "enabled"
	}
	...
	if dm.RuntimeEnforcer == nil {
		// exception: no LSM
		if pod.Annotations["kubedig-policy"] == "enabled" {
			pod.Annotations["kubedig-policy"] = "audited"
		}
	...
	if _, ok := pod.Annotations["kubedig-visibility"]; !ok {
		pod.Annotations["kubedig-visibility"] = cfg.GlobalCfg.Visibility
	}
	...
	if event == addEvent || event == updateEvent {
		...
		for _, k8spod := range dm.K8sPods {
			if k8spod.Metadata["namespaceName"] == pod.Metadata["namespaceName"] && k8spod.Metadata["podName"] == pod.Metadata["podName"] {
				if k8spod.Annotations["kubedig-policy"] == "patched" {
					exist = true
					break
				}
			}
	...
	if dm.RuntimeEnforcer != nil && dm.RuntimeEnforcer.EnforcerType == "AppArmor" {
		...
		for _, container := range obj.Spec.Containers {
			...
			profileName := "kubedig-" + pod.Metadata["namespaceName"] + "-" + podOwnerName + "-" + container.Name
		...
		if event == addEvent {
			...
			if updateAppArmor && pod.Annotations["kubedig-policy"] == "enabled" && dm.OwnerInfo[pod.Metadata["podName"]].Ref != "Pod" {

				// patch deployments only when kubedig-controller is not present
				if dm.OwnerInfo[pod.Metadata["podName"]].Name != "" && cfg.GlobalCfg.AnnotateResources {
					...
					pod.Annotations["kubedig-policy"] = "patched"
				}
			}
			...
			for _, k8spod := range dm.K8sPods {
				if k8spod.Metadata["namespaceName"] == pod.Metadata["namespaceName"] && k8spod.Metadata["podName"] == pod.Metadata["podName"] {
					...
					if val, ok := k8spod.Annotations["kubedig-policy"]; ok {
						prevPolicyEnabled = val
					}

					if updateAppArmor && prevPolicyEnabled != "enabled" && pod.Annotations["kubedig-policy"] == "enabled" && dm.OwnerInfo[pod.Metadata["podName"]].Ref != "Pod" {
	...
	if pod.Annotations["kubedig-policy"] == "patched" {
...
func (dm *KubeDigDaemon) WatchK8sPods() {
...
func (dm *KubeDigDaemon) GetSecurityPolicies(endPoint tp.EndPoint) []tp.SecurityPolicy {
...
func (dm *KubeDigDaemon) UpdateSecurityPolicy(action string, secPolicyType string, secPolicy tp.SecurityPolicy) {
	dm.EndPointsLock.RLock()
	endPointsLength := len(dm.EndPoints)
	dm.EndPointsLock.RUnlock()
	...
	for idx := 0; idx < endPointsLength; idx++ {
		...
		if secPolicyType == KubeDigPolicy {
			if len(secPolicy.Spec.Selector.Containers) == 0 || kl.ContainsElement(secPolicy.Spec.Selector.Containers, endPoint.ContainerName) {
				...
				if cfg.GlobalCfg.Policy {
					...
					if dm.EndPoints[idx].PolicyEnabled == tp.KubeDigPolicyEnabled {
						// enforce security policies
		...
		} else if secPolicyType == KubeDigClusterPolicy {
...
// CreateSecurityPolicy - creates `KubeDigPolicy` & `KubeDigClusterPolicy` object from crd
func (dm *KubeDigDaemon) CreateSecurityPolicy(policyType string, securityPolicy interface{}) (secPolicy tp.SecurityPolicy, err error) {
	var namespace, name string

	if policyType == KubeDigPolicy {
		kubedigPolicy := securityPolicy.(ksp.KubeDigPolicy)

		namespace = kubedigPolicy.Namespace
		name = kubedigPolicy.Name
		if err := kl.Clone(kubedigPolicy.Spec, &secPolicy.Spec); err != nil {
		...
		for k, v := range secPolicy.Spec.Selector.MatchLabels {
			if k == "kubedig.io/container.name" {
				if len(v) > 2 {
	...
	} else if policyType == KubeDigClusterPolicy {
		kubedigClusterPolicy := securityPolicy.(ksp.KubeDigClusterPolicy)

		namespace = kubedigClusterPolicy.Namespace
		name = kubedigClusterPolicy.Name
		if err := kl.Clone(kubedigClusterPolicy.Spec, &secPolicy.Spec); err != nil {
			dm.Logger.Errf("Failed to clone a spec (%s)", err.Error())
			return tp.SecurityPolicy{}, err
		}
...
func (dm *KubeDigDaemon) WatchSecurityPolicies() cache.InformerSynced {
	for {
		if !K8s.CheckCustomResourceDefinition("kubedigpolicies") {
	...
	informer := factory.Security().V1().KubeDigPolicies().Informer()
	registration, err := informer.AddEventHandler(
		cache.ResourceEventHandlerFuncs{
			AddFunc: func(obj interface{}) {
				// create a security policy
				if policy, ok := obj.(*ksp.KubeDigPolicy); ok {

					secPolicy, err := dm.CreateSecurityPolicy(KubeDigPolicy, *policy)
					if err != nil {
					...
					dm.UpdateSecurityPolicy(addEvent, KubeDigPolicy, secPolicy)

				}
			},
			UpdateFunc: func(oldObj, newObj interface{}) {
				if policy, ok := newObj.(*ksp.KubeDigPolicy); ok {
					secPolicy, err := dm.CreateSecurityPolicy(KubeDigPolicy, *policy)
					if err != nil {
						return
					}
					...
					// apply security policies to pods
					dm.UpdateSecurityPolicy(updateEvent, KubeDigPolicy, secPolicy)
				}
			},
			DeleteFunc: func(obj interface{}) {
				if policy, ok := obj.(*ksp.KubeDigPolicy); ok {
					...
					// apply security policies to pods
					dm.UpdateSecurityPolicy(deleteEvent, KubeDigPolicy, secPolicy)
				}
			},
		},
	)
	if err != nil {
		dm.Logger.Err("Couldn't start watching KubeDig Security Policies")
		return nil
	}
...
func (dm *KubeDigDaemon) WatchClusterSecurityPolicies(timeout time.Duration) cache.InformerSynced {
	...
	for !crdFound {
		select {
		case <-ctx.Done():
			dm.Logger.Warn("timeout while monitoring cluster security policies, kubedigclusterpolicies CRD not found")
			return nil
		default:
			if K8s.CheckCustomResourceDefinition("kubedigclusterpolicies") {
				crdFound = true
			} else {
				time.Sleep(time.Second * 1)
			}
	...
	informer := factory.Security().V1().KubeDigClusterPolicies().Informer()
	registration, err := informer.AddEventHandler(
		cache.ResourceEventHandlerFuncs{
			AddFunc: func(obj interface{}) {
				// create a security policy
				if policy, ok := obj.(*ksp.KubeDigClusterPolicy); ok {

					secPolicy, err := dm.CreateSecurityPolicy(KubeDigClusterPolicy, *policy)
					if err != nil {
						dm.Logger.Warnf("Error ADD, %s", err)
						return
					}
					...
					// apply security policies to pods
					dm.UpdateSecurityPolicy(addEvent, KubeDigClusterPolicy, secPolicy)

				}
			},
			UpdateFunc: func(oldObj, newObj interface{}) {
				if policy, ok := newObj.(*ksp.KubeDigClusterPolicy); ok {
					secPolicy, err := dm.CreateSecurityPolicy(KubeDigClusterPolicy, *policy)
					if err != nil {
						return
					}
					...
					// apply security policies to pods
					dm.UpdateSecurityPolicy(updateEvent, KubeDigClusterPolicy, secPolicy)
				}
			},
			DeleteFunc: func(obj interface{}) {
				if policy, ok := obj.(*ksp.KubeDigClusterPolicy); ok {
					...
					// apply security policies to pods
					dm.UpdateSecurityPolicy(deleteEvent, KubeDigClusterPolicy, secPolicy)
				}
			},
		},
	)
...
func (dm *KubeDigDaemon) UpdateHostSecurityPolicies() {
	...
	if cfg.GlobalCfg.HostPolicy {
		...
		if dm.RuntimeEnforcer != nil {
			if dm.Node.PolicyEnabled == tp.KubeDigPolicyEnabled {
...
func (dm *KubeDigDaemon) ParseAndUpdateHostSecurityPolicy(event tp.K8sKubeDigHostPolicyEvent) pb.PolicyStatus {
	...
	if !cfg.GlobalCfg.K8sEnv && (cfg.GlobalCfg.KVMAgent || cfg.GlobalCfg.HostPolicy) {
		if event.Type == addEvent || event.Type == updateEvent {
			...
			dm.backupKubeDigHostPolicy(secPolicy)
		} else if event.Type == deleteEvent {
...
func (dm *KubeDigDaemon) WatchHostSecurityPolicies(timeout time.Duration) {
	ctx, cancel := context.WithTimeout(context.Background(), timeout)
	defer cancel()
	...
	for {
		select {
		case <-ctx.Done():
			dm.Logger.Warn("timeout while monitoring host security policies, kubedighostpolicies CRD not found")
			return
		default:
			if !K8s.CheckCustomResourceDefinition("kubedighostpolicies") {
				time.Sleep(time.Second * 1)
				continue
			}
		}
		...
		if resp := K8s.WatchK8sHostSecurityPolicies(); resp != nil {
			...
			for {
				event := tp.K8sKubeDigHostPolicyEvent{}
...
func (dm *KubeDigDaemon) updatEndpointsWithCM(cm *corev1.ConfigMap, action string) {
	dm.EndPointsLock.Lock()
	...
	for _, ns := range nsList.Items {
		ns := ns
		fp, fa := validateDefaultPosture("kubedig-file-posture", &ns, cm.Data[cfg.ConfigDefaultFilePosture])
		np, na := validateDefaultPosture("kubedig-network-posture", &ns, cm.Data[cfg.ConfigDefaultNetworkPosture])
		cp, ca := validateDefaultPosture("kubedig-capabilities-posture", &ns, cm.Data[cfg.ConfigDefaultCapabilitiesPosture])
...
func (dm *KubeDigDaemon) UpdateDefaultPostureWithCM(endPoint *tp.EndPoint, action string, namespace string, defaultPosture tp.DefaultPosture, annotated bool) {
...
func (dm *KubeDigDaemon) UpdateDefaultPosture(action string, namespace string, defaultPosture tp.DefaultPosture, annotated bool) {
	dm.DefaultPosturesLock.Lock()
	defer dm.DefaultPosturesLock.Unlock()
...
func (dm *KubeDigDaemon) validateVisibility(scope string, visibility string) bool {
	return strings.Contains(visibility, scope)
}

// UpdateVisibility Function
func (dm *KubeDigDaemon) UpdateVisibility(action string, namespace string, visibility tp.Visibility) {
...
var visibilityKey string = "kubedig-visibility"

func (dm *KubeDigDaemon) updateVisibilityWithCM(cm *corev1.ConfigMap, action string) {
...
func (dm *KubeDigDaemon) UpdateGlobalPosture(posture tp.DefaultPosture) {
	dm.EndPointsLock.Lock()
	defer dm.EndPointsLock.Unlock()
...
func (dm *KubeDigDaemon) WatchDefaultPosture() cache.InformerSynced {
	...
	registration, err := informer.AddEventHandler(cache.ResourceEventHandlerFuncs{
		AddFunc: func(obj interface{}) {
			if ns, ok := obj.(*corev1.Namespace); ok {
				fp, fa := validateDefaultPosture("kubedig-file-posture", ns, cfg.GlobalCfg.DefaultFilePosture)
				np, na := validateDefaultPosture("kubedig-network-posture", ns, cfg.GlobalCfg.DefaultNetworkPosture)
				cp, ca := validateDefaultPosture("kubedig-capabilities-posture", ns, cfg.GlobalCfg.DefaultCapabilitiesPosture)
		...
		DeleteFunc: func(obj interface{}) {
			if ns, ok := obj.(*corev1.Namespace); ok {
				_, fa := validateDefaultPosture("kubedig-file-posture", ns, cfg.GlobalCfg.DefaultFilePosture)
				_, na := validateDefaultPosture("kubedig-network-posture", ns, cfg.GlobalCfg.DefaultNetworkPosture)
				_, ca := validateDefaultPosture("kubedig-capabilities-posture", ns, cfg.GlobalCfg.DefaultCapabilitiesPosture)
				annotated := fa || na || ca
...
func (dm *KubeDigDaemon) WatchConfigMap() cache.InformerSynced {
	configMapLabelOption := informers.WithTweakListOptions(func(opts *metav1.ListOptions) {
		opts.LabelSelector = fmt.Sprintf("kubedig-app=%s", "kubedig-configmap")
	})
...
// GetConfigMapNS Returns KubeDig configmap namespace
func (dm *KubeDigDaemon) GetConfigMapNS() string {
	// get namespace from env
	envNamespace := os.Getenv("KUBEDIG_NAMESPACE")

	if envNamespace == "" {
		// kubedig is running as system process,
		// return "kubedig" for testing purpose in dev env
		return "kubedig"
	}
	return envNamespace
}
