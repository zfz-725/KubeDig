...
import (
	...
	kspclient "github.com/zfz-725/KubeDig/pkg/KubeDigController/client/clientset/versioned"
)
...
func (kh *K8sHandler) PatchResourceWithAppArmorAnnotations(namespaceName, deploymentName string, appArmorAnnotations map[string]string, kind string) error {
	...
	spec := `{"spec":{"template":{"metadata":{"annotations":{"kubedig-policy":"enabled",`
	if kind == "CronJob" {
		spec = `{"spec":{"jobTemplate":{"spec":{"template":{"metadata":{"annotations":{"kubedig-policy":"enabled",`
	}
	...
	if resBody, errOut := kh.DoRequest("GET", nil, "/apis"); errOut == nil {
		...
		if errIn := json.Unmarshal(resBody, &res); errIn == nil {
			for _, group := range res.Groups {
				if group.Name == "security.kubedig.com" {
					exist = true
					apiGroup = group
					break
				}
...
func (kh *K8sHandler) WatchK8sSecurityPolicies() *http.Response {
	...
	if kl.IsInK8sCluster() {
		URL := "https://" + kh.K8sHost + ":" + kh.K8sPort + "/apis/security.kubedig.com/v1/kubedigpolicies?watch=true"
	...
	// kube-proxy (local)
	URL := "http://" + kh.K8sHost + ":" + kh.K8sPort + "/apis/security.kubedig.com/v1/kubedigpolicies?watch=true"
...
func (kh *K8sHandler) WatchK8sHostSecurityPolicies() *http.Response {
	...
	if kl.IsInK8sCluster() {
		URL := "https://" + kh.K8sHost + ":" + kh.K8sPort + "/apis/security.kubedig.com/v1/kubedighostpolicies?watch=true"
	...
	// kube-proxy (local)
	URL := "http://" + kh.K8sHost + ":" + kh.K8sPort + "/apis/security.kubedig.com/v1/kubedighostpolicies?watch=true"
