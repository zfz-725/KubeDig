...
// SetKdigData generates runtime configuration for KubeDig to be consumed by kDig
func (dm *KubeDigDaemon) SetKdigData() {
	var kd KdigData
	...
	kd.KernelHeaderPresent = true //this is always true since KubeDig is running
	kd.HostVisibility = dm.Node.Annotations["kubedig-visibility"]
	err := kl.WriteToFile(kd, "/tmp/kdigProbeData.cfg")
	if err != nil {
		dm.Logger.Errf("Error writing kdig config data (%s)", err.Error())
	}

}

// SetProbeContainerData keeps track of containers and the applied policies
func (dm *KubeDigDaemon) SetProbeContainerData() ([]string, map[string]*pb.ContainerData, map[string]*pb.HostSecurityPolicies) {
