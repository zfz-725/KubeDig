...
func (dm *KubeDigDaemon) NewNRIHandler(
	handleDeletedContainer func(tp.Container),
	handleNewContainer func(tp.Container),
) *NRIHandler {
...
func (nh *NRIHandler) Synchronize(
	...
	for _, nriContainer := range nriContainers {
		container := nh.nriToKubeDigContainer(nriContainer)
...
// KubeDig is no longer enforcing anything on it while it shuts down.
...
// nriToKubeDigContainer converts an NRI container to a KubeDig container.
func (nh *NRIHandler) nriToKubeDigContainer(nriContainer *api.Container) tp.Container {
	...
	if nh.dm.K8sEnabled {
		...
		container.AppArmorProfile = "kubedig_" + container.ContainerName
	}
...
func (dm *KubeDigDaemon) MonitorNRIEvents() {
	dm.WgDaemon.Add(1)
