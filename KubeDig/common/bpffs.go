...
var (
	...
	fallbackBPFFsPath = "/run/kubedig/bpffs"
...
//     that kubedig is running inside a container and the host
...
func checkOrMountDefaultLocations() error {
	...
	if !bpffsInstance {
		...
		// than BPFFS, it means that Kubedig is running inside
		// container and /sys/fs/bpf is not mounted on host. So, we
		// mount BPFFS in /run/kubedig/bpffs inside the container.
		// This will allow operation of Kubedig but will result in
		// unmounting of the filesystem when the pod is restarted.
		kg.Warnf("BPF filesystem is going to be mounted automatically "+
			"in %s. However, it probably means that Kubedig is running "+
