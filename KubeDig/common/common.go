...
import (
	...
	kc "github.com/zfz-725/KubeDig/KubeDig/config"
...
func MatchIdentities(identities []string, superIdentities []string) bool {
	...
	for _, identity := range identities {
		...
		if strings.Contains(identity, "kubedig.io/container.name") {
			if !MatchesRegex("kubedig.io/container.name", identity, superIdentities) {
				matched = false
				break
			}
		...
		if strings.Contains(identity, "kubedig.io/hostname") {
			if !MatchesRegex("kubedig.io/hostname", identity, superIdentities) {
				matched = false
				break
			}
