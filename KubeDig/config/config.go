...
// Package config is the component responsible for loading KubeDig configurations
package config
...
// KubedigConfig Structure
type KubedigConfig struct {
	...
	Debug bool // Enable/Disable KubeDig debug mode
	...
	StateAgent bool // enable KubeDig state agent
	...
	AnnotateResources bool  // enable annotations by kubedig if kubedig-controller is not present

	ProcFsMount string // path where procfs is hosted
}

// GlobalCfg Global configuration for Kubedig
var GlobalCfg KubedigConfig

// Config const
const (
	PolicyDir                            string = "/opt/kubedig/policies/"
	PIDFilePath                          string = "/opt/kubedig/kubedig.pid"
	...
	ConfigKubedigPolicy                string = "enableKubeDigPolicy"
	ConfigKubedigHostPolicy            string = "enableKubeDigHostPolicy"
	ConfigKubedigVM                    string = "enableKubeDigVm"
	...
	ConfigStateAgent                     string = "enableKubeDigStateAgent"
...
func readCmdLineParams() {
	...
	tlsCertsStr := flag.String(ConfigTLSCertPath, "/var/lib/kubedig/tls", "path to tls ca certificate files ca.crt, ca.crt")
	...
	seLinuxProfileDirStr := flag.String(ConfigSELinuxProfileDir, "/tmp/kubedig.selinux", "SELinux profile directory")
	...
	policyB := flag.Bool(ConfigKubedigPolicy, true, "enabling KubeDigPolicy")
	hostPolicyB := flag.Bool(ConfigKubedigHostPolicy, false, "enabling KubeDigHostPolicy")
	kvmAgentB := flag.Bool(ConfigKubedigVM, false, "enabling KubeDigVM")
	k8sEnvB := flag.Bool(ConfigK8sEnv, true, "is k8s env?")

	debugB := flag.Bool(ConfigDebug, false, "Enable/Disable pushing KubeDig debug logs over gRPC. NOTE: Set environment DEBUG=true to configure stdout debug logging")
	...
	untrackedNs := flag.String(ConfigUntrackedNs, "kube-system,kubedig", "Namespaces which are not being tracked, default untracked:[kube-system, kubedig]")
	...
	stateAgent := flag.Bool(ConfigStateAgent, false, "enabling KubeDig State Agent client")
	...
	annotateResources := flag.Bool(ConfigAnnotateResources, false, "for kubedig deployment without kubedig-controller")
	...
	viper.SetDefault(ConfigKubedigPolicy, *policyB)
	viper.SetDefault(ConfigKubedigHostPolicy, *hostPolicyB)
	viper.SetDefault(ConfigKubedigVM, *kvmAgentB)
...
func LoadConfig() error {
	...
	// Note that the env var has to be set in uppercase for e.g, CLUSTER=xyz ./kubedig
	viper.AutomaticEnv()

	// Read configuration from config file
	cfgfile := os.Getenv("KUBEDIG_CFG")
	if cfgfile == "" {
		cfgfile = "kubedig.yaml"
	}
	...
	GlobalCfg.Policy = viper.GetBool(ConfigKubedigPolicy)
	GlobalCfg.HostPolicy = viper.GetBool(ConfigKubedigHostPolicy)
	GlobalCfg.KVMAgent = viper.GetBool(ConfigKubedigVM)
...
// LoadDynamicConfig set dynamic configuration which can be updated at runtime without restarting kubedig
