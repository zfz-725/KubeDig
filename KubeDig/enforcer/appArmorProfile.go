...
func (ae *AppArmorEnforcer) GenerateAppArmorProfile(appArmorProfile string, securityPolicies []tp.SecurityPolicy, defaultPosture tp.DefaultPosture, privileged bool) (int, string, bool) {
	...
	if strings.Contains(appArmorProfile, "kubedig.host") {
		if _, err := os.Stat(filepath.Clean("/etc/apparmor.d/" + "kubedig.host")); os.IsNotExist(err) {
			return 0, err.Error(), false
		}

		// get the old profile
		profile, err := os.ReadFile(filepath.Clean("/etc/apparmor.d/" + "kubedig.host"))
		if err != nil {
			return 0, err.Error(), false
		}
