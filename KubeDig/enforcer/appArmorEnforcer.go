...
func NewAppArmorEnforcer(node tp.Node, logger *fd.Feeder) *AppArmorEnforcer {
	...
	// default profile
	ae.ApparmorDefault = `## == Managed by KubeDig == ##
...
`
	...
	ae.ApparmorDefaultPrivileged = `## == Managed by KubeDig == ##
#include <tunables/global>
...
`
	...
	ae.rgx = regexp.MustCompile("profile kubedig-.* {")
			...
			if f.IsDir() {
				if _, err := strconv.Atoi(f.Name()); err == nil {
					if content, err := os.ReadFile(filepath.Clean(cfg.GlobalCfg.ProcFsMount + "/" + f.Name() + "/attr/current")); err == nil {
						...
						// check if there are KubeDig's profiles used by containers
	...
	for _, file := range files {
		...
		if strings.Contains(str, "KubeDig") {
...
func (ae *AppArmorEnforcer) RegisterAppArmorProfile(podName, profileName string, privileged bool) bool {
	...
	if _, err := os.Stat(filepath.Clean("/etc/apparmor.d/" + profileName)); err == nil {
		if content, err := os.ReadFile(filepath.Clean("/etc/apparmor.d/" + profileName)); err != nil {
			...
			return false
		} else if !strings.Contains(string(content), "KubeDig") {
...
const appArmorHostFile = "/etc/apparmor.d/kubedig.host"

// ClearKubeDigHostFile Function
func (ae *AppArmorEnforcer) ClearKubeDigHostFile(fileName string) {
	/* Remove contents of AppArmor profile once the policy is applied
	 * This will prevent reboot issues related to ungraceful shutdown of kubedig
	 */
...
func (ae *AppArmorEnforcer) CreateAppArmorHostProfile() error {
	...
	apparmorHostDefault := `## == Managed by KubeDig == ##
#include <tunables/global>

profile kubedig.host /{usr/,}bin/*sh flags=(attach_disconnected,mediate_deleted) {
## == PRE START == ##
...
`
	...
	if err != nil {
		ae.Logger.Warnf("Unable to open the KubeDig host profile in %s (%s)", cfg.GlobalCfg.Host, err.Error())
		return err
	}

	if _, err := newfile.WriteString(apparmorHostDefault); err != nil {
		ae.Logger.Warnf("Unable to update the KubeDig host profile in %s (%s)", cfg.GlobalCfg.Host, err.Error())
		return err
	}

	if err := newfile.Close(); err != nil {
		ae.Logger.Warnf("Unable to close the KubeDig host profile in %s (%s)", cfg.GlobalCfg.Host, err.Error())
		return err
	}
...
func (ae *AppArmorEnforcer) RegisterAppArmorHostProfile() bool {
	...
	if err := ae.CreateAppArmorHostProfile(); err != nil {
		ae.Logger.Warnf("Unable to create the KubeDig host profile in %s (%s)", cfg.GlobalCfg.Host, err.Error())
		return false
	}

	if err := kl.RunCommandAndWaitWithErr("apparmor_parser", []string{"-r", "-W", "-C", appArmorHostFile}); err != nil {
		ae.Logger.Warnf("Unable to register the KubeDig host profile in %s (%s)", cfg.GlobalCfg.Host, err.Error())
		return false
	}

	ae.Logger.Printf("Registered the KubeDig host profile in %s", cfg.GlobalCfg.Host)

	ae.ClearKubeDigHostFile(appArmorHostFile)
...
func (ae *AppArmorEnforcer) UnregisterAppArmorHostProfile() bool {
	...
	ae.Logger.Printf("Unregistering the KubeDig host profile from %s", cfg.GlobalCfg.Host)

	ae.AppArmorProfilesLock.Lock()
	defer ae.AppArmorProfilesLock.Unlock()

	if err := kl.RunCommandAndWaitWithErr("aa-remove-unknown", []string{}); err != nil {
		ae.Logger.Warnf("Unable to cleanup the KubeDig host profile in %s", cfg.GlobalCfg.Host)

		if err := ae.CreateAppArmorHostProfile(); err != nil {
			ae.Logger.Warnf("Unable to reset the KubeDig host profile in %s", cfg.GlobalCfg.Host)

			if err := os.Remove(appArmorHostFile); err != nil {
				ae.Logger.Warnf("Unable to remove the KubeDig host profile from %s (%s)", cfg.GlobalCfg.Host, err.Error())
			}
	...
	ae.Logger.Printf("Unregistered the KubeDig host profile from %s", cfg.GlobalCfg.Host)
...
func (ae *AppArmorEnforcer) UpdateAppArmorHostProfile(secPolicies []tp.HostSecurityPolicy) {
	...
	if policyCount, newProfile, ok := ae.GenerateAppArmorProfile("kubedig.host /{usr/,}bin/*sh", hostPolicies, globalDefaultPosture, true); ok {
		newfile, err := os.Create(filepath.Clean(appArmorHostFile))
		if err != nil {
			ae.Logger.Warnf("Unable to open the KubeDig host profile in %s (%s)", cfg.GlobalCfg.Host, err.Error())
			return
		...
		if err := newfile.Sync(); err != nil {
			ae.Logger.Warnf("Unable to sync the KubeDig host profile in %s (%s)", cfg.GlobalCfg.Host, err.Error())
		...
		if err := kl.RunCommandAndWaitWithErr("apparmor_parser", []string{"-r", "-W", appArmorHostFile}); err != nil {
			ae.Logger.Warnf("Unable to update %d host security rule(s) to the KubeDig host profile in %s (%s)", policyCount, cfg.GlobalCfg.Host, err.Error())
			return
		}

		ae.Logger.Printf("Updated %d host security rules to the KubeDig host profile in %s", policyCount, cfg.GlobalCfg.Host)
