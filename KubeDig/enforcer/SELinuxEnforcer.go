...
func NewSELinuxEnforcer(node tp.Node, logger *fd.Feeder) *SELinuxEnforcer {
	...
	if ok := se.InstallSELinuxModulesIfNeeded(); !ok {
		se.Logger.Warnf("Unable to install SELinux modules required by the KubeDig host profile in %s", cfg.GlobalCfg.Host)
		return nil
	}
...
func (se *SELinuxEnforcer) RegisterSELinuxHostProfile() bool {
	...
	if err != nil {
		se.Logger.Warnf("Unable to create the KubeDig host profile in %s (%s)", cfg.GlobalCfg.Host, err.Error())
		return false
	}

	if _, err := newFile.WriteString(""); err != nil {
		se.Logger.Warnf("Unable to update the KubeDig host profile in %s (%s)", cfg.GlobalCfg.Host, err.Error())
		return false
	}

	if err := newFile.Close(); err != nil {
		se.Logger.Warnf("Unable to close the KubeDig host profile in %s (%s)", cfg.GlobalCfg.Host, err.Error())
		return false
	}

	if ok := se.UpdateSELinuxLabels(cfg.GlobalCfg.SELinuxProfileDir + se.HostProfile); !ok {
		se.Logger.Warnf("Unable to register the KubeDig host profile in %s", cfg.GlobalCfg.Host)
		return false
	}

	se.Logger.Printf("Registered the KubeDig host profile in %s", cfg.GlobalCfg.Host)
...
func (se *SELinuxEnforcer) UnregisterSELinuxHostProfile() bool {
	...
	newFile, err := os.Create(filepath.Clean(cfg.GlobalCfg.SELinuxProfileDir + se.HostProfile))
	if err != nil {
		se.Logger.Warnf("Unable to open the KubeDig host profile in %s (%s)", cfg.GlobalCfg.Host, err.Error())
		return false
	}
	...
	if ok := se.UpdateSELinuxLabels(cfg.GlobalCfg.SELinuxProfileDir + se.HostProfile); !ok {
		se.Logger.Warnf("Unable to unregister the KubeDig host profile in %s", cfg.GlobalCfg.Host)
		return false
	}

	se.Logger.Printf("Unregistered the KubeDig host profile from %s", cfg.GlobalCfg.Host)
...
func (se *SELinuxEnforcer) UpdateSELinuxHostProfile(secPolicies []tp.HostSecurityPolicy) {
	...
	if policyCount, newProfile, ok := se.GenerateSELinuxHostProfile(secPolicies, globalDefaultPosture); ok {
		...
		if err := newFile.Sync(); err != nil {
			se.Logger.Warnf("Unable to sync the KubeDig host profile in %s (%s)", cfg.GlobalCfg.Host, err.Error())
		...
		if ok := se.UpdateSELinuxLabels(cfg.GlobalCfg.SELinuxProfileDir + se.HostProfile); !ok {
			se.Logger.Warnf("Unable to update SELinux labels affected by the KubeDig host profile in %s", cfg.GlobalCfg.Host)
			return
		}

		se.Logger.Printf("Updated %d host security rule(s) to the KubeDig host profile in %s", policyCount, cfg.GlobalCfg.Host)
	}
}
...
func (se *SELinuxEnforcer) InstallSELinuxModulesIfNeeded() bool {
	...
	for _, line := range strings.Split(res, "\n") {
		...
		if len(words) == 3 && strings.HasPrefix(words[0], "kdig") {
			modules = append(modules, words[0])
		}
	}

	if kl.ContainsElement(modules, "kdig") {
		return true
	}
