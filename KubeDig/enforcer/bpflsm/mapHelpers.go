	...
	if be.ContainerMap[containerID].Map != nil {
		if err := be.BPFContainerMap.Delete(be.ContainerMap[containerID].Key); err != nil {
			if !errors.Is(err, os.ErrNotExist) {
				be.Logger.Errf("error deleting container %s from outer map in kubedig_containers map: %s", containerID, err.Error())
			}
		}
		if err := be.BPFContainerThrottleMap.Delete(be.ContainerMap[containerID].Key); err != nil {
			if !errors.Is(err, os.ErrNotExist) {
				be.Logger.Errf("error deleting container %s from outer map in kubedig_alert_throttle map: %s", containerID, err.Error())
			}
		}
