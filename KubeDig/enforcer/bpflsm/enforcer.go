...
import (
	...
	"github.com/zfz-725/KubeDig/KubeDig/buildinfo"
...
func NewBPFEnforcer(node tp.Node, pinpath string, logger *fd.Feeder, monitor *mon.SystemMonitor) (*BPFEnforcer, error) {
	...
	be.BPFContainerMap, err = ebpf.NewMapWithOptions(&ebpf.MapSpec{
		...
		Name:       "kubedig_containers",
	}, ebpf.MapOptions{
		PinPath: pinpath,
	})
	if err != nil {
		be.Logger.Errf("error creating kubedig_containers map: %s", err)
		return be, err
	}
	...
	be.BPFContainerThrottleMap, err = ebpf.NewMapWithOptions(&ebpf.MapSpec{
		...
		MaxEntries: 256,
		Pinning:    ebpf.PinByName,
		Name:       "kubedig_alert_throttle",
	}, ebpf.MapOptions{
		PinPath: pinpath,
	})
	if err != nil {
		be.Logger.Errf("error creating kubedig_alert_throttle map: %s", err)
		return be, err
	}
	...
	*/
	...
	be.Events, err = ringbuf.NewReader(be.obj.KubedigEvents)
	if err != nil {
...
func (be *BPFEnforcer) TraceEvents() {
	...
	for {
		...
		log.KubeDigVersion = buildinfo.GitSummary
		log.Enforcer = "BPFLSM"
...
func (be *BPFEnforcer) DestroyBPFEnforcer() error {
	...
	if be.Events != nil {
		if err := be.obj.KubedigEvents.Unpin(); err != nil {
			be.Logger.Err(err.Error())
			errBPFCleanUp = errors.Join(errBPFCleanUp, err)
		}
		if err := be.obj.KubedigEvents.Close(); err != nil {
			be.Logger.Err(err.Error())
			errBPFCleanUp = errors.Join(errBPFCleanUp, err)
		}
