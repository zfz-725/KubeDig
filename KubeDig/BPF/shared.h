...
struct outer_hash kubedig_containers SEC(".maps");
...
static __always_inline bool should_drop_alerts_per_container(struct outer_key okey) {
  ...
  u64 maxAlert = (u64)get_kubedig_config(_MAX_ALERT_PER_SEC); 
  ...
  if (state->event_count > maxAlert) {
    ...
    event *event_data = bpf_ringbuf_reserve(&kubedig_events, sizeof(event), 0);
    if (!event_data) {
      // Failed to reserve
      return true;
    }
...
static inline int match_and_enforce_path_hooks(struct path *f_path, u32 id,
  ...
  u32 *inner = bpf_map_lookup_elem(&kubedig_containers, &okey);
...
ringbuf:
  if (get_kubedig_config(_ALERT_THROTTLING) && should_drop_alerts_per_container(okey)) {
    return retval;
  }

  task_info = bpf_ringbuf_reserve(&kubedig_events, sizeof(event), 0);
  if (!task_info) {
