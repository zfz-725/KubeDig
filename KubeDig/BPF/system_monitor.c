...
#define KBUILD_MODNAME "kubedig_system_monitor"
#endif

// ========================================================== //
// KubeDig utilizes Tracee's system call handling functions //
...
struct visibility kubedig_visibility SEC(".maps");
...
struct kaconfig kubedig_config SEC(".maps");
...
struct exec_pid_map kubedig_exec_pids SEC(".maps");
...
static __always_inline u32 get_kubedig_config(u32 config)
{
    u32 *value = bpf_map_lookup_elem(&kubedig_config, &config);
    if (!value)
    {
        return 0;
    }
...
static __always_inline u32 add_pid_ns()
{
    ...
    if (pid_ns == PROC_PID_INIT_INO && get_kubedig_config(_MONITOR_HOST))
    { // host
    ...
    else if(get_kubedig_config(_MONITOR_CONTAINER))
    { // container
...
static __always_inline u32 remove_pid_ns()
{
    ...
    { // container
        if (get_task_ns_pid(task) == 1)
        {
            ...
            bpf_map_delete_elem(&kubedig_alert_throttle, &key);
...
static __always_inline u32 drop_syscall(u32 scope)
{
    ...
    u32 *d_visibility = bpf_map_lookup_elem(&kubedig_visibility, &defaultvizkey);
    if (d_visibility) {
    ...
    u32 *ns_visibility = bpf_map_lookup_elem(&kubedig_visibility, &okey);
    if (!ns_visibility)
    {
...
static __always_inline u32 init_context(sys_context_t *context)
{
    ...
    { // container
        ...
        u64 *exec_id = bpf_map_lookup_elem(&kubedig_exec_pids, &host_pid);
        if (exec_id) {
...
static __always_inline bool should_drop_alerts_per_container(sys_context_t *context, struct pt_regs *ctx, u32 types, args_t *args) {
    ...
    struct alert_throttle_state *state = bpf_map_lookup_elem(&kubedig_alert_throttle, &key);
    ...
    if (!state) {
        ...
        bpf_map_update_elem(&kubedig_alert_throttle, &key, &new_state, BPF_ANY);
        return false;
    }

    u64 throttle_sec = (u64)get_kubedig_config(_THROTTLE_SEC);
    u64 throttle_nsec = throttle_sec * 1000000000L;
    u64 max = (u64)get_kubedig_config(_MAX_ALERT_PER_SEC);
    ...
    if (state->event_count > max) {
        state->event_count = 0;
        state->throttle = 1;
        bpf_map_update_elem(&kubedig_alert_throttle, &key, state, BPF_ANY);
...
int sys_exit_setns(struct tracepoint_raw_sys_exit *ctx)
{
    ...
    if (pre_ns_data->mnt_ns != new_mnt_ns || 
        ...
        u32 *matches = bpf_map_lookup_elem(&kubedig_visibility, &key);
        u64 exec_id = bpf_ktime_get_ns() | pid;
        if (matches) {   
          bpf_map_update_elem(&kubedig_exec_pids, &pid, &exec_id, BPF_ANY);
        }
...
int sched_process_fork(struct tracepoint_sched_process_fork *ctx)
{
    ...
    u32 *exists = bpf_map_lookup_elem(&kubedig_exec_pids, &parent_pid);
    if (exists) {
        // to make verifier happy on older kernel versions i.e. 4.15
        u32 val = *exists;
        bpf_map_update_elem(&kubedig_exec_pids, &child_pid, &val, BPF_ANY);
    }
...
int kprobe__security_bprm_check(struct pt_regs *ctx)
{
    ...
    if (get_kubedig_config(_ENFORCER_BPFLSM) && drop_syscall(_PROCESS_PROBE))
    {
        return 0;
    }
...
int kretprobe__execve(struct pt_regs *ctx)
{
    ...
    u32 types;
    args_t args = {};
    if (context.retval < 0 && !get_kubedig_config(_ENFORCER_BPFLSM) && get_kubedig_config(_ALERT_THROTTLING) && should_drop_alerts_per_container(&context, ctx, types, &args))
    {
        return 0;
    }
...
int kprobe__do_exit(struct pt_regs *ctx)
{
    ...
    bpf_map_delete_elem(&kubedig_exec_pids, &tgid);
...
static __always_inline int trace_ret_generic(u32 id, struct pt_regs *ctx, u64 types, u32 scope)
{
    ...
    if (get_kubedig_config(_ENFORCER_BPFLSM) && drop_syscall(scope))
    {
...
int sys_exit_openat(struct tracepoint_syscalls_sys_exit_t *args)
{
    ...
    if (get_kubedig_config(_ENFORCER_BPFLSM) && drop_syscall(_FILE_PROBE))
    {
        // dropping after load_args so as the args_map cleanup happens
        return 0;
    }
    ...
    if (context.retval < 0 && !get_kubedig_config(_ENFORCER_BPFLSM) && get_kubedig_config(_ALERT_THROTTLING) && should_drop_alerts_per_container(&context, (struct pt_regs *)args, types, &orig_args))
    {
        return 0;
    }
...
int kretprobe__tcp_connect(struct pt_regs *ctx)
{
    ...
    if (get_kubedig_config(_ENFORCER_BPFLSM) && drop_syscall(_NETWORK_PROBE))
    {
        return 0;
    }
...
int kprobe__udp_sendmsg(struct pt_regs *ctx)
{
    ...
    if (get_kubedig_config(_ENFORCER_BPFLSM) && drop_syscall(_DNS_PROBE))
    {
        return 0;
    }
