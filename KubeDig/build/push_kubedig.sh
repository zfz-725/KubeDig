...
cd $ARMOR_HOME/..; docker buildx build --metadata-file kubedig.json --platform $PLATFORMS --target kubedig -t $REPO:$VERSION -f Dockerfile $BUILD_MODE $LABEL $STABEL_LABEL .
...
cd $ARMOR_HOME/..; docker buildx build --metadata-file kubedig-init.json --platform $PLATFORMS --build-arg VERSION=$VERSION -t $REPO-init:$VERSION -f Dockerfile.init $BUILD_MODE $LABEL $STABEL_LABEL .
...
cd $ARMOR_HOME/..; docker buildx build --metadata-file kubedig-ubi.json --platform $PLATFORMS --build-arg VERSION=$VERSION --target kubedig-ubi -t $UBIREPO:$VERSION -f Dockerfile $BUILD_MODE $LABEL $STABEL_LABEL .
