...
import (
	...
	klog "github.com/zfz-725/kubedig-client/log"
...
// KarmorGetTargetLogs waits for logs from kubedig. KarmorLogStart() has to be called
...
// KarmorLogStart start observing for kubedig telemetry events
func KarmorLogStart(logFilter string, ns string, op string, pod string) error {
	...
	go func() {
		err := klog.StartObserver(k8sClient, klog.Options{
			...
			TlsCertPath:      "/var/lib/kubedig/tls",
...
// KarmorGetLogs waits for logs from kubedig. KarmorQueueLog() has to be called
// before this so that the channel is established.
...
// KarmorLogStop stops the kubedig-client observer
func KarmorLogStop() {
...
// IsOperationsExpected validates what KubeDig Operation is expected based on visibility configuration
