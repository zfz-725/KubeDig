...
var _ = Describe("Visibility", func() {
	...
	Describe("Visibility tests", func() {
		It("can get full visibility", func() {
			err := util.AnnotateNS("wordpress-mysql", "kubedig-visibility", "file,process,network,capabilities")
			Expect(err).To(BeNil())
		...
		It("can get only network visibility", func() {
			err := util.AnnotateNS("wordpress-mysql", "kubedig-visibility", "network")
			Expect(err).To(BeNil())
		...
		It("can drop all visibility data", func() {
			err := util.AnnotateNS("wordpress-mysql", "kubedig-visibility", "none")
			Expect(err).To(BeNil())
