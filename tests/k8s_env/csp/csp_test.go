...
var _ = Describe("csp", func() {
	...
	BeforeEach(func() {
		n1 = getNginxPod("nginx1-", "nginx1", "kubedig-policy: enabled")
		n2 = getNginxPod("nginx2-", "nginx2", "kubedig-policy: enabled")
	})
	...
	Describe("Policy Apply", func() {
		It("can block execution of pkg mgmt tools apt, In operator", func() {
			...
			nl := getNginxPod("nginx-later-deploy", "nginx-later", "kubedig-policy: enabled")
