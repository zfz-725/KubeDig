...
function start_and_wait_for_kubedig_initialization() {
    ...
    if [ "$DEFAULT_POSTURE" == "allow" ]; then
        ...
        ARMOR_OPTIONS+=("-defaultFilePosture=audit")
        ARMOR_OPTIONS+=("-defaultNetworkPosture=audit")
        ARMOR_OPTIONS+=("-defaultCapabilitiesPosture=audit")
    else # block
    ...
    make clean; make
    sudo -E ./kubedig -logPath=$ARMOR_LOG ${ARMOR_OPTIONS[@]} > $ARMOR_MSG &
    echo "Executed KubeDig"

    for (( ; ; ))
    do
        grep "Initialized KubeDig" $ARMOR_MSG &> /dev/null
        [[ $? -eq 0 ]] && break
        sleep 1
    done
}
