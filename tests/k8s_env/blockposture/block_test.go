...
var _ = BeforeSuite(func() {
	...
	_, err = Kubectl(fmt.Sprintf("annotate ns wordpress-mysql kubedig-network-posture=block --overwrite"))
	Expect(err).To(BeNil())

	_, err = Kubectl(fmt.Sprintf("annotate ns wordpress-mysql kubedig-file-posture=block --overwrite"))
	Expect(err).To(BeNil())
...
var _ = Describe("Posture", func() {
	...
	BeforeEach(func() {
		wp = getWpsqlPod("wordpress-", "kubedig-policy: enabled")
	})
	...
	Describe("Policy Apply", func() {
		...
		It("can whitelist certain files accessed by a package while blocking all other sensitive content", func() {
			err := util.AnnotateNS("wordpress-mysql", "kubedig-network-posture", "block")
			Expect(err).To(BeNil())
			// Apply policy
