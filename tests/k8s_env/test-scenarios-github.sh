...
ARMOR_HOME=`dirname $(realpath "$0")`/../KubeDig
...
case $1 in
    "-testPolicy")
        ...
        ;;
    "-testHostPolicy")
        echo "If you want to test host policies, please run KubeDig separately and use test-scenarios-in-runtime.sh"
        exit
        ;;
    "-testNativePolicy")
...
function start_and_wait_for_kubedig_initialization() {
    cd $CRD_HOME
    ...
    if [[ ! " ${ARMOR_OPTIONS[@]} " =~ "-enableKubeDigHostPolicy" ]]; then
        SKIP_HOST_POLICY=1
    fi

    ka_podname=`kubectl get pods -n kubedig -l kubedig-app=kubedig -o custom-columns=":metadata.name" --no-headers`
    if [ "$ka_podname" != "" ]; then
        echo "Found KubeDig from Kubernetes"

        CAT_LOG="kubectl exec -n kubedig $ka_podname -- cat $ARMOR_LOG"
        CAT_MSG="kubectl logs -n kubedig $ka_podname"

        sleep 10

        for count in {1..120}
        do
            $CAT_MSG | grep "Initialized KubeDig$" &> /dev/null
            [[ $? -eq 0 ]] && break

            $CAT_MSG | grep "Terminated KubeDig$" &> /dev/null
            [[ $? -eq 0 ]] && $CAT_MSG && exit 1
        ...
        $CAT_MSG
    else # start kubedig as local process
        echo "Not found KubeDig from Kubernetes, executing KubeDig as a local process"
        ...
        sudo -E ./kubedig -test.coverprofile=.coverprofile -logPath=$ARMOR_LOG ${ARMOR_OPTIONS[@]} > $ARMOR_MSG &
        echo "Executed KubeDig"
...
function stop_and_wait_for_kubedig_termination() {
    [[ "$ka_podname" != "" ]] && echo "kubedig not started by this script, hence not stopping" && return
    ps -e | grep kubedig | awk '{print $1}' | xargs -I {} sudo kill {}

    for count in {1..60}
    do
        ps -e | grep kubedig &> /dev/null
        if [ $? != 0 ]; then
            break
        fi
...
INFO "Starting KubeDig"
start_and_wait_for_kubedig_initialization
INFO "Started KubeDig"
...
INFO "Stopping KubeDig"
stop_and_wait_for_kubedig_termination
DBG "Stopped KubeDig"
