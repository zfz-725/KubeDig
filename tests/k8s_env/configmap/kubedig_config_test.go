...
var _ = Describe("KubeDig-Config", func() {
	var unannotated string
	var partialyAnnotated string
	var fullyAnnotated string
	BeforeEach(func() {
		unannotated = getUnannotatedPod("unannotated-", "kubedig-policy: enabled")
		partialyAnnotated = getPartialyAnnotatedPod("partialyannotated-", "kubedig-policy: enabled")
		fullyAnnotated = getFullyAnnotatedPod("fullyannotated-", "kubedig-policy: enabled")
		cm := NewDefaultConfigMapData()
		cm.CreateKAConfigMap()
	})
