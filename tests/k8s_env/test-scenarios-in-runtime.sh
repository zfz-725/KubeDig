...
case $1 in
    ...
    "-testNativePolicy")
        if [ "$LSM" != "apparmor" ]; then
            echo "KubeDig does not support native policies if AppArmor is not enabled"
            exit
        fi
...
ARMOR_LOG=/tmp/kubedig.log
TEST_LOG=/tmp/kubedig.test
...
function should_not_find_any_log() {
    ...
    KUBEDIG=$(kubectl get pods -n kubedig -l kubedig-app=kubedig -o wide 2> /dev/null | grep $NODE | grep kubedig | awk '{print $1}')

    if [[ $KUBEDIG = "kubedig"* ]]; then
        audit_log=$(kubectl -n kubedig exec $KUBEDIG -- grep -E "$1.*policyName.*$2.*MatchedPolicy.*$6.*operation.*$3.*resource.*$4.*data.*action.*$5" $ARMOR_LOG | grep -v grep | tail -n 1 | grep -v Passed)
        if [ $? == 0 ]; then
            echo $audit_log
...
function should_find_passed_log() {
    ...
    if [[ $KUBEDIG = "kubedig"* ]]; then
        audit_log=$(kubectl -n kubedig exec $KUBEDIG -- grep -E "$1.*policyName.*$2.*MatchedPolicy.*operation.*$3.*resource.*$4.*data.*action.*$5" $ARMOR_LOG | grep -v grep | tail -n 1 | grep Passed)
        if [ $? != 0 ]; then
            audit_log="<No Log>"
...
function should_find_blocked_log() {
    ...
    if [[ $KUBEDIG = "kubedig"* ]]; then
        audit_log=$(kubectl -n kubedig exec $KUBEDIG -- grep -E "$1.*policyName.*$2.*MatchedPolicy.*operation.*$3.*resource.*$4.*data.*action.*$5" $ARMOR_LOG | grep -v grep | tail -n 1 | grep -v Passed)
        if [ $? != 0 ]; then
            audit_log="<No Log>"
            FAIL "Failed to find the log from logs"
            res_cmd=1
...
function should_not_find_any_host_log() {
    ...
    if [[ $KUBEDIG = "kubedig"* ]]; then
        audit_log=$(kubectl -n kubedig exec $KUBEDIG -- grep -E "$HOST_NAME.*policyName.*$1.*MatchedHostPolicy.*$5.*operation.*$2.*resource.*$3.*data.*action.*$4" $ARMOR_LOG | grep -v grep | tail -n 1 | grep -v Passed)
        if [ $? == 0 ]; then
            echo $audit_log
            FAIL "Found the log from logs"
            res_cmd=1
...
function should_find_passed_host_log() {
    ...
    if [[ $KUBEDIG = "kubedig"* ]]; then
        audit_log=$(kubectl -n kubedig exec $KUBEDIG -- grep -E "$HOST_NAME.*policyName.*$1.*MatchedHostPolicy.*operation.*$2.*resource.*$3.*data.*action.*$4" $ARMOR_LOG | grep -v grep | tail -n 1 | grep Passed)
        if [ $? != 0 ]; then
            audit_log="<No Log>"
            FAIL "Failed to find the log from logs"
            res_cmd=1
...
function should_find_blocked_host_log() {
    ...
    if [[ $KUBEDIG = "kubedig"* ]]; then
        audit_log=$(kubectl -n kubedig exec $KUBEDIG -- grep -E "$HOST_NAME.*policyName.*$1.*MatchedHostPolicy.*operation.*$2.*resource.*$3.*data.*action.*$4" $ARMOR_LOG | grep -v grep | tail -n 1 | grep -v Passed)
        if [ $? != 0 ]; then
            audit_log="<No Log>"
            FAIL "Failed to find the log from logs"
            res_cmd=1
...
if [[ ! "$(ps -f --pid $(pidof kubedig) 2> /dev/null | cat | grep enableKubeDigHostPolicy)" != "" ]]; then
    SKIP_HOST_POLICY=1
fi
...
echo "< KubeDig Test Report >" > $TEST_LOG
echo >> $TEST_LOG
...
if [[ $res_microservice -eq 1 ]] || [[ $res_host -eq 1 ]]; then
    FAIL "Failed to test KubeDig"
    exit 1
fi

PASS "Successfully tested KubeDig"
exit 0
