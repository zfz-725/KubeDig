...
func LoadSeccompInNode() {
	...
	if err := os.WriteFile(seccompPath+"/kubedig-seccomp.json", jsonData, 0664); err != nil {
		log.Errf("Error copying file to seccomp directory: %s", err.Error())
	}

	if err := os.WriteFile(seccompPath+"/kubedig-init-seccomp.json", jsonInitData, 0664); err != nil {
		log.Errf("Error copying file to seccomp directory: %s", err.Error())
	}
}

func CheckIfSeccompProfilePresent() string {
	_, err1 := os.Stat(filepath.Clean(seccompPath + "/kubedig-init-seccomp.json"))
	_, err2 := os.Stat(filepath.Clean(seccompPath + "/kubedig-seccomp.json"))
