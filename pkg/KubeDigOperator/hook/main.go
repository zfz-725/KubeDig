...
var (
	kubeDigSocket string
	runtimeSocket   string
	detached        bool
)

func main() {
	flag.StringVar(&kubeDigSocket, "kubedig-socket", "/var/run/kubedig/ka.sock", "KubeDig socket")
...
func run(state specs.State) error {
	...
	// the decision whether a container is KubeDig container or not is done
	// based on two things:
	...
	// this might lead to some containers acting as KubeDig to spawn a detached
	// processes that are unneeded. However, the design of KubeDig hook logic
	...
	// but we always need to make sure to never introduce any false negatives as KubeDig
	...
	if err != nil {
		...
		isKubeDig = containerName == "kubedig"
	} else {
		var spec specs.Spec
		...
		isKubeDig = spec.Process.Args[0] == "/KubeDig/kubedig"
	}
	...
	if isKubeDig {
		...
		// to make sure if it was a false positive (container trying to act as KubeDig), we still
		// monitor it.
	}
...
func sendContainer(container types.Container, operation types.HookOperation) error {
	conn, err := net.Dial("unix", kubeDigSocket)
	if err != nil {
		// not returning error here because this can happen in multiple cases
		// that we don't want container creation to be blocked on:
		// - hook was created before KubeDig was running so the socket doesn't exist yet
		// - KubeDig crashed so there is nothing listening on socket
		return nil
	}
