			...
			UpdateFunc: func(oldObj, newObj interface{}) {
				if cfg, ok := newObj.(*opv1.KubeDigConfig); ok {
					...
					if common.OperatorConfigCrd != nil && cfg.Name == common.OperatorConfigCrd.Name {
						...
						if !tlsUpdated && !relayEnvUpdated && !configChanged && cfg.Status != oldObj.(*opv1.KubeDigConfig).Status && len(imageUpdated) < 1 {
							return
						}
						if tlsUpdated {
...
func (clusterWatcher *ClusterWatcher) UpdateKubeDigImages(images []string) error {
	...
	for _, img := range images {
		...
		case "controller", "rbac":
			controller, err := clusterWatcher.Client.AppsV1().Deployments(common.Namespace).Get(context.Background(), deployments.KubeDigControllerDeploymentName, v1.GetOptions{})
			if err != nil {
