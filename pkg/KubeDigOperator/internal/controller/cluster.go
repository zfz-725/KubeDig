...
import (
	...
	secv1 "github.com/zfz-725/KubeDig/pkg/KubeDigController/api/security.kubedig.com/v1"
	secv1client "github.com/zfz-725/KubeDig/pkg/KubeDigController/client/clientset/versioned"
	opv1 "github.com/zfz-725/KubeDig/pkg/KubeDigOperator/api/operator.kubedig.com/v1"
	"github.com/zfz-725/KubeDig/pkg/KubeDigOperator/cert"
	opv1client "github.com/zfz-725/KubeDig/pkg/KubeDigOperator/client/clientset/versioned"
	"github.com/zfz-725/KubeDig/pkg/KubeDigOperator/client/clientset/versioned/scheme"
	opv1Informer "github.com/zfz-725/KubeDig/pkg/KubeDigOperator/client/informers/externalversions"
	"github.com/zfz-725/KubeDig/pkg/KubeDigOperator/common"
	"github.com/zfz-725/KubeDig/pkg/KubeDigOperator/recommend"
	"go.uber.org/zap"
...
var deployment_name string = "kubedig-operator"
...
func (clusterWatcher *ClusterWatcher) WatchConfigCrd() {
	...
	informer := factory.Operator().V1().KubeDigConfigs().Informer()

	if informer == nil {
		clusterWatcher.Log.Warn("Failed to initialize KubeDigConfig informer")
		return
	}
	...
	informer.AddEventHandler(
		cache.ResourceEventHandlerFuncs{
			AddFunc: func(obj interface{}) {
				configCrdList, err := clusterWatcher.Opv1Client.OperatorV1().KubeDigConfigs(common.Namespace).List(context.Background(), metav1.ListOptions{})
				if err != nil {
			...
			UpdateFunc: func(oldObj, newObj interface{}) {
				if cfg, ok := newObj.(*opv1.KubeDigConfig); ok {
					...
					if common.OperatorConfigCrd != nil && cfg.Name == common.OperatorConfigCrd.Name {
						...
						relayEnvUpdated := UpdatedKubedigRelayEnv(&cfg.Spec)
						seccompEnabledUpdated := UpdatedSeccomp(&cfg.Spec)
						tlsUpdated := UpdateTlsData(&cfg.Spec)
						UpdateRecommendedPolicyConfig(&cfg.Spec)

						// return if only status has been updated
						if !tlsUpdated && !relayEnvUpdated && !configChanged && cfg.Status != oldObj.(*opv1.KubeDigConfig).Status && len(imageUpdated) < 1 && !controllerPortUpdated {
							return
						}
						if tlsUpdated {
						...
						if len(imageUpdated) > 0 {
							clusterWatcher.UpdateKubeDigImages(imageUpdated)
						}
						if configChanged {
							// update status to Updating
							go clusterWatcher.UpdateCrdStatus(cfg.Name, common.UPDATING, common.UPDATING_MSG)
							clusterWatcher.UpdateKubeDigConfigMap(cfg)
						}
						if relayEnvUpdated {
							// update status to Updating
							go clusterWatcher.UpdateCrdStatus(cfg.Name, common.UPDATING, common.UPDATING_MSG)
							clusterWatcher.UpdateKubedigRelayEnv(cfg)
						}
						if seccompEnabledUpdated {
							go clusterWatcher.UpdateCrdStatus(cfg.Name, common.UPDATING, common.UPDATING_MSG)
							clusterWatcher.UpdateKubedigSeccomp(cfg)
						}
						if controllerPortUpdated {
							clusterWatcher.UpdateKubeDigImages([]string{"controller"})
...
func (clusterWatcher *ClusterWatcher) UpdateKubeDigImages(images []string) error {
	var res error
	for _, img := range images {
		...
		case "kubedig", "init":
			...
			if err != nil {
				clusterWatcher.Log.Warnf("Cannot list KubeDig daemonset(s) error=%s", err.Error())
				res = err
			} else {
				for _, ds := range dsList.Items {
					ds.Spec.Template.Spec.Containers[0].Image = common.GetApplicationImage(common.KubeDigName)
					ds.Spec.Template.Spec.Containers[0].ImagePullPolicy = corev1.PullPolicy(common.KubeDigImagePullPolicy)
					ds.Spec.Template.Spec.Containers[0].Args = common.KubeDigArgs
					ds.Spec.Template.Spec.ImagePullSecrets = common.KubeDigImagePullSecrets
					...
					ds.Spec.Template.Spec.Tolerations = common.KubeDigTolerations
					...
					if len(ds.Spec.Template.Spec.InitContainers) != 0 {
						ds.Spec.Template.Spec.InitContainers[0].Image = common.GetApplicationImage(common.KubeDigInitName)
						ds.Spec.Template.Spec.InitContainers[0].ImagePullPolicy = corev1.PullPolicy(common.KubeDigInitImagePullPolicy)
						ds.Spec.Template.Spec.InitContainers[0].Args = common.KubeDigInitArgs
						ds.Spec.Template.Spec.ImagePullSecrets = append(ds.Spec.Template.Spec.ImagePullSecrets, common.KubeDigInitImagePullSecrets...)
						ds.Spec.Template.Spec.Tolerations = append(ds.Spec.Template.Spec.Tolerations, common.KubeDigInitTolerations...)
					}

					NRIVolume, NRIVolumeMount := common.GenerateNRIvol(ds.Spec.Selector.MatchLabels["kubedig.io/nri-socket"])
		...
		case "relay":
			...
			if err != nil {
				...
				relay.Spec.Template.Spec.Containers[0].Image = common.GetApplicationImage(common.KubeDigRelayName)
				relay.Spec.Template.Spec.Containers[0].ImagePullPolicy = corev1.PullPolicy(common.KubeDigRelayImagePullPolicy)
				relay.Spec.Template.Spec.Containers[0].Args = common.KubeDigRelayArgs
				relay.Spec.Template.Spec.ImagePullSecrets = common.KubeDigRelayImagePullSecrets
				...
				relay.Spec.Template.Spec.Tolerations = common.KubeDigRelayTolerations
				...
				if err != nil {
					...
					clusterWatcher.Log.Infof("Updated Deployment=%s with image=%s", deployments.RelayDeploymentName, common.KubeDigRelayImage)
				}
			}

		case "controller", "rbac":
			dep, err := clusterWatcher.Client.AppsV1().Deployments(common.Namespace).Get(context.Background(), deployments.KubeDigControllerDeploymentName, v1.GetOptions{})
			if err != nil {
				clusterWatcher.Log.Warnf("Cannot get deployment=%s error=%s", deployments.KubeDigControllerDeploymentName, err.Error())
				res = err
			} else {
				controller := dep.DeepCopy()
				controller.Spec.Template.Spec.ImagePullSecrets = common.KubeDigControllerImagePullSecrets
				...
				controller.Spec.Template.Spec.Tolerations = common.KubeDigControllerTolerations
				...
				for i, container := range *containers {
					if container.Name == "manager" {
						(*containers)[i].Image = common.GetApplicationImage(common.KubeDigControllerName)
						(*containers)[i].ImagePullPolicy = corev1.PullPolicy(common.KubeDigControllerImagePullPolicy)
						(*containers)[i].Args = common.KubeDigControllerArgs
						(*containers)[i].Ports[0].ContainerPort = int32(common.KubeDigControllerPort)
					}
				}
				UpdateArgsIfDefinedAndUpdated(&controller.Spec.Template.Spec.Containers[0].Args, []string{"webhook-port=" + strconv.Itoa(common.KubeDigControllerPort)})

				_, err := clusterWatcher.Client.AppsV1().Deployments(common.Namespace).Update(context.Background(), controller, v1.UpdateOptions{})
				if err != nil {
					clusterWatcher.Log.Warnf("Cannot update deployment=%s error=%s", deployments.KubeDigControllerDeploymentName, err.Error())
					res = err
				} else {
					clusterWatcher.Log.Infof("Updated Deployment=%s", deployments.KubeDigControllerDeploymentName)
				}
			}
		}
	}
...
func (clusterWatcher *ClusterWatcher) UpdateKubedigRelayEnv(cfg *opv1.KubeDigConfig) error {
	var res error
	relay, err := clusterWatcher.Client.AppsV1().Deployments(common.Namespace).Get(context.Background(), deployments.RelayDeploymentName, v1.GetOptions{})
	if err != nil {
		clusterWatcher.Log.Warnf("Cannot get deployment=%s error=%s", deployments.RelayDeploymentName, err.Error())
		...
		relay.Spec.Template.Spec.Containers[0].Env = []corev1.EnvVar{
			{
				...
				Value: common.KubedigRelayEnvMap[common.EnableStdOutLogs],
			},
			{
				Name:  "ENABLE_STDOUT_ALERTS",
				Value: common.KubedigRelayEnvMap[common.EnableStdOutAlerts],
			},
			{
				Name:  "ENABLE_STDOUT_MSGS",
				Value: common.KubedigRelayEnvMap[common.EnableStdOutMsgs],
			},
			{
		...
		if err != nil {
			...
			clusterWatcher.Log.Infof("Updated Deployment=%s with env=%s", deployments.RelayDeploymentName, common.KubedigRelayEnvMap)
		}
	}
	return res
}

func (clusterWatcher *ClusterWatcher) UpdateKubedigSeccomp(cfg *opv1.KubeDigConfig) error {
	var res error
	dsList, err := clusterWatcher.Client.AppsV1().DaemonSets(common.Namespace).List(context.Background(), v1.ListOptions{
		LabelSelector: "kubedig-app=kubedig",
	})
...
func UpdateImages(config *opv1.KubeDigConfigSpec) []string {
	updatedImages := []string{}
	// if kubedig image or imagePullPolicy got updated
	if UpdateIfDefinedAndUpdated(&common.KubeDigImage, config.KubeDigImage.Image) ||
		UpdateIfDefinedAndUpdated(&common.KubeDigImagePullPolicy, config.KubeDigImage.ImagePullPolicy) ||
		UpdateArgsIfDefinedAndUpdated(&common.KubeDigArgs, config.KubeDigImage.Args) ||
		UpdateImagePullSecretsIfDefinedAndUpdated(&common.KubeDigImagePullSecrets, config.KubeDigImage.ImagePullSecrets) ||
		UpdateTolerationsIfDefinedAndUpdated(&common.KubeDigTolerations, config.KubeDigImage.Tolerations) ||
		UpdateNRIAvailabilityIfDefinedAndUpdated(&common.NRIEnabled, config.EnableNRI) {
		updatedImages = append(updatedImages, "kubedig")
	}
	// if kubedig-init image or imagePullPolicy got updated
	if UpdateIfDefinedAndUpdated(&common.KubeDigInitImage, config.KubeDigInitImage.Image) ||
		UpdateIfDefinedAndUpdated(&common.KubeDigInitImagePullPolicy, config.KubeDigInitImage.ImagePullPolicy) ||
		UpdateArgsIfDefinedAndUpdated(&common.KubeDigInitArgs, config.KubeDigInitImage.Args) ||
		UpdateImagePullSecretsIfDefinedAndUpdated(&common.KubeDigInitImagePullSecrets, config.KubeDigInitImage.ImagePullSecrets) ||
		UpdateTolerationsIfDefinedAndUpdated(&common.KubeDigInitTolerations, config.KubeDigInitImage.Tolerations) {
		updatedImages = append(updatedImages, "init")
	}
	// kubedig-relay image or imagePullPolicy got updated
	if UpdateIfDefinedAndUpdated(&common.KubeDigRelayImage, config.KubeDigRelayImage.Image) ||
		UpdateIfDefinedAndUpdated(&common.KubeDigRelayImagePullPolicy, config.KubeDigRelayImage.ImagePullPolicy) ||
		UpdateArgsIfDefinedAndUpdated(&common.KubeDigRelayArgs, config.KubeDigRelayImage.Args) ||
		UpdateImagePullSecretsIfDefinedAndUpdated(&common.KubeDigRelayImagePullSecrets, config.KubeDigRelayImage.ImagePullSecrets) ||
		UpdateTolerationsIfDefinedAndUpdated(&common.KubeDigRelayTolerations, config.KubeDigRelayImage.Tolerations) {
		updatedImages = append(updatedImages, "relay")
	}
	// if kubedig-controller image or imagePullPolicy got updated
	if UpdateIfDefinedAndUpdated(&common.KubeDigControllerImage, config.KubeDigControllerImage.Image) ||
		UpdateIfDefinedAndUpdated(&common.KubeDigControllerImagePullPolicy, config.KubeDigControllerImage.ImagePullPolicy) ||
		UpdateArgsIfDefinedAndUpdated(&common.KubeDigControllerArgs, config.KubeDigControllerImage.Args) ||
		UpdateImagePullSecretsIfDefinedAndUpdated(&common.KubeDigControllerImagePullSecrets, config.KubeDigControllerImage.ImagePullSecrets) ||
		UpdateTolerationsIfDefinedAndUpdated(&common.KubeDigControllerTolerations, config.KubeDigControllerImage.Tolerations) {
	...
	if UpdateImagePullSecretsIfDefinedAndUpdated(&common.GlobalImagePullSecrets, config.GloabalImagePullSecrets) ||
		...
		updatedImages = []string{"kubedig", "init", "relay", "controller"}
	}
...
func (clusterWatcher *ClusterWatcher) UpdateCrdStatus(cfg, phase, message string) {
	err := wait.ExponentialBackoff(wait.Backoff{Steps: 5, Duration: 500 * time.Millisecond}, func() (bool, error) {
		configCrd, err := clusterWatcher.Opv1Client.OperatorV1().KubeDigConfigs(common.Namespace).Get(context.Background(), cfg, metav1.GetOptions{})
		if err != nil {
			// retry the update
			return false, nil
		}
		newStatus := opv1.KubeDigConfigStatus{
			Phase:   phase,
			Message: message,
		}
		...
		if configCrd.Status != newStatus {
			...
			_, err = clusterWatcher.Opv1Client.OperatorV1().KubeDigConfigs(common.Namespace).UpdateStatus(context.Background(), configCrd, metav1.UpdateOptions{})
			if err != nil {
				// retry the update
				return false, nil
			}
...
func (clusterWatcher *ClusterWatcher) UpdateKubeDigConfigMap(cfg *opv1.KubeDigConfig) {
	err := wait.ExponentialBackoff(wait.Backoff{Steps: 5, Duration: 500 * time.Millisecond}, func() (bool, error) {
		cm, err := clusterWatcher.Client.CoreV1().ConfigMaps(common.Namespace).Get(context.Background(), deployments.KubeDigConfigMapName, metav1.GetOptions{})
		if err != nil {
			if isNotfound(err) {
				return true, nil
			}
	...
	if err != nil {
		clusterWatcher.Log.Errorf("Error updating the KubeDig Configmap %s", err)
	...
	clusterWatcher.Log.Info("KubeDig Config Updated Successfully")
}
...
func (clusterWatcher *ClusterWatcher) WatchTlsState(tlsEnabled bool) error {
	...
	relay, err := clusterWatcher.Client.AppsV1().Deployments(common.Namespace).Get(context.Background(), deployments.RelayDeploymentName, v1.GetOptions{})
	if err != nil {
		clusterWatcher.Log.Warnf("Cannot list KubeDig relay deployment error=%s", err.Error())
		return err
	}
	dsList, err := clusterWatcher.Client.AppsV1().DaemonSets(common.Namespace).List(context.Background(), v1.ListOptions{
		LabelSelector: "kubedig-app=kubedig",
	...
	if tlsEnabled {
		if cert.CACert != nil {
			ca, err := clusterWatcher.Client.CoreV1().Secrets(common.Namespace).Get(context.Background(), common.KubeDigCaSecretName, v1.GetOptions{})
...
func (clusterWatcher *ClusterWatcher) UpdateWebhookSvcPort(port int) {
	...
	svc, err := clusterWatcher.Client.CoreV1().Services(common.Namespace).Get(context.Background(), common.KubeDigControllerWebhookServiceName, v1.GetOptions{})
	if err != nil {
		clusterWatcher.Log.Warnf("Cannot get webhook service=%s error=%s", common.KubeDigControllerWebhookServiceName, err.Error())
	} else {
		...
		if err != nil {
			clusterWatcher.Log.Warnf("Cannot update webhook service=%s error=%s", common.KubeDigControllerWebhookServiceName, err.Error())
		} else {
			clusterWatcher.Log.Infof("Updated webhook service=%s", common.KubeDigControllerWebhookServiceName)
		}
	}
}
...
func (clusterWatcher *ClusterWatcher) UpdateTlsVolumeAndVolumeMounts(action string) error {
	...
	// configure relay deployment and kubedig daemonset
	relay, err := clusterWatcher.Client.AppsV1().Deployments(common.Namespace).Get(context.Background(), deployments.RelayDeploymentName, v1.GetOptions{})
	if err != nil {
		clusterWatcher.Log.Warnf("Cannot get deployment=%s error=%s", deployments.RelayDeploymentName, err.Error())
		return err
		...
		common.AddOrRemoveVolumeMount(&common.KubeDigRelayTlsVolumeMount, &relay.Spec.Template.Spec.Containers[0].VolumeMounts, action)
		common.AddOrRemoveVolume(&common.KubeDigRelayTlsVolume, &relay.Spec.Template.Spec.Volumes, action)
	...
	if err != nil {
		...
		for _, ds := range dsList.Items {
			// update daemonset volumeMount and volumes
			common.AddOrRemoveVolumeMount(&common.KubeDigCaVolumeMount, &ds.Spec.Template.Spec.Containers[0].VolumeMounts, action)
			common.AddOrRemoveVolume(&common.KubeDigCaVolume, &ds.Spec.Template.Spec.Volumes, action)
...
func (clusterWatcher *ClusterWatcher) DeleteAllTlsSecrets() error {
	...
	tlsSecrets := []string{
		common.KubeDigCaSecretName,
		common.KubeDigClientSecretName,
		common.KubeDigRelayServerSecretName,
	}
...
func (clusterWatcher *ClusterWatcher) WatchRecommendedPolicies() error {
	...
	for _, policy := range policies {
		csp := &secv1.KubeDigClusterPolicy{}
		if !policy.IsDir() {
		...
		case true:
			if slices.Contains(common.RecommendedPolicies.ExcludePolicy, csp.Name) {
				...
				err = clusterWatcher.Secv1Client.SecurityV1().KubeDigClusterPolicies().Delete(context.Background(), csp.GetName(), metav1.DeleteOptions{})
				if err != nil && !metav1errors.IsNotFound(err) {
			...
			csp.Annotations["app.kubernetes.io/managed-by"] = "kubedig-operator"
			_, err = clusterWatcher.Secv1Client.SecurityV1().KubeDigClusterPolicies().Create(context.Background(), csp, metav1.CreateOptions{})
			if err != nil && !metav1errors.IsAlreadyExists(err) {
				...
				pol, err := clusterWatcher.Secv1Client.SecurityV1().KubeDigClusterPolicies().Get(context.Background(), csp.GetName(), metav1.GetOptions{})
				if err != nil {
				...
				if !reflect.DeepEqual(pol.Spec.Selector.MatchExpressions, common.RecommendedPolicies.MatchExpressions) {
					...
					_, err := clusterWatcher.Secv1Client.SecurityV1().KubeDigClusterPolicies().Update(context.Background(), pol, metav1.UpdateOptions{})
					if err != nil {
		...
		case false:
			if !policy.IsDir() {
				if pol, err := clusterWatcher.Secv1Client.SecurityV1().KubeDigClusterPolicies().Get(context.Background(), csp.Name, v1.GetOptions{}); err == nil &&
					pol.Annotations["app.kubernetes.io/managed-by"] == "kubedig-operator" {
					err = clusterWatcher.Secv1Client.SecurityV1().KubeDigClusterPolicies().Delete(context.Background(), csp.GetName(), metav1.DeleteOptions{})
					if err != nil && !metav1errors.IsNotFound(err) {
						clusterWatcher.Log.Warnf("error deleting csp %s", csp.GetName())
						continue
...
func UpdateRecommendedPolicyConfig(config *opv1.KubeDigConfigSpec) bool {
	updated := false
...
func UpdateConfigMapData(config *opv1.KubeDigConfigSpec) bool {
	updated := false
	configMapData := ""
	...
	common.ConfigMapData[common.KubeDigConfigFileName] = configMapData

	return updated
}

func UpdatedKubedigRelayEnv(config *opv1.KubeDigConfigSpec) bool {
	updated := false
	stringEnableStdOutLogs := strconv.FormatBool(config.EnableStdOutLogs)
	if stringEnableStdOutLogs != "" {
		if common.KubedigRelayEnvMap[common.EnableStdOutLogs] != stringEnableStdOutLogs {
			common.KubedigRelayEnvMap[common.EnableStdOutLogs] = stringEnableStdOutLogs
			updated = true
		}
	}
	...
	if stringEnableStdOutAlerts != "" {
		if common.KubedigRelayEnvMap[common.EnableStdOutAlerts] != stringEnableStdOutAlerts {
			common.KubedigRelayEnvMap[common.EnableStdOutAlerts] = stringEnableStdOutAlerts
			updated = true
		}
	}
	...
	if stringEnableStdOutMsgs != "" {
		if common.KubedigRelayEnvMap[common.EnableStdOutMsgs] != stringEnableStdOutMsgs {
			common.KubedigRelayEnvMap[common.EnableStdOutMsgs] = stringEnableStdOutMsgs
			updated = true
		}
	}
...
func UpdatedSeccomp(config *opv1.KubeDigConfigSpec) bool {
	updated := false
...
func UpdateTlsData(config *opv1.KubeDigConfigSpec) bool {
	updated := false
...
func UpdateControllerPort(config *opv1.KubeDigConfigSpec) bool {
	updated := false
	if config.ControllerPort != 0 && config.ControllerPort != common.KubeDigControllerPort {

		common.ControllerPortLock.Lock()
		common.KubeDigControllerPort = config.ControllerPort
