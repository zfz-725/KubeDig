...
import (
	...
	crds "github.com/zfz-725/KubeDig/pkg/KubeDigController/crd"
	certGen "github.com/zfz-725/KubeDig/pkg/KubeDigOperator/cert"
...
func generateDaemonset(name, enforcer, runtime, socket, nriSocket, btfPresent, apparmorfs, seccompPresent string, initDeploy bool, ociHooks string) *appsv1.DaemonSet {
	...
	labels["kubedig-app"] = "kubedig"
	daemonset.Spec.Template.Labels = labels
	daemonset.Spec.Template.Spec.ServiceAccountName = "kubedig"
	...
	// TODO: handle passing annotateResource flag to kubedig
	...
	if common.EnableTls {
		vols = append(vols, common.KubeDigCaVolume...)
		volMnts = append(volMnts, common.KubeDigCaVolumeMount...)
	...
	if btfPresent == "no" || initDeploy {
		...
		daemonset.Spec.Template.Spec.InitContainers[0].Image = common.GetApplicationImage(common.KubeDigInitName)
		daemonset.Spec.Template.Spec.InitContainers[0].ImagePullPolicy = corev1.PullPolicy(common.KubeDigInitImagePullPolicy)
		UpdateArgsIfDefinedAndUpdated(&daemonset.Spec.Template.Spec.InitContainers[0].Args, common.KubeDigInitArgs)
		UpdateImagePullSecretsIfDefinedAndUpdated(&daemonset.Spec.Template.Spec.ImagePullSecrets, common.KubeDigInitImagePullSecrets)
		UpdateTolerationsIfDefinedAndUpdated(&daemonset.Spec.Template.Spec.Tolerations, common.KubeDigInitTolerations)
	}
	// update images
	...
	daemonset.Spec.Template.Spec.Containers[0].Image = common.GetApplicationImage(common.KubeDigName)
	daemonset.Spec.Template.Spec.Containers[0].ImagePullPolicy = corev1.PullPolicy(common.KubeDigImagePullPolicy)
	UpdateArgsIfDefinedAndUpdated(&daemonset.Spec.Template.Spec.Containers[0].Args, common.KubeDigArgs)
	UpdateImagePullSecretsIfDefinedAndUpdated(&daemonset.Spec.Template.Spec.ImagePullSecrets, common.KubeDigImagePullSecrets)
	UpdateTolerationsIfDefinedAndUpdated(&daemonset.Spec.Template.Spec.Tolerations, common.KubeDigInitTolerations)
...
func genSnitchRole() *rbacv1.ClusterRole {
	return &rbacv1.ClusterRole{
		ObjectMeta: metav1.ObjectMeta{
			Name:      common.KubeDigSnitchRoleName,
...
func genSnitchRoleBinding() *rbacv1.ClusterRoleBinding {
	return &rbacv1.ClusterRoleBinding{
		ObjectMeta: metav1.ObjectMeta{
			Name: common.KubeDigSnitchRoleName + "-binding",
		},
...
func deploySnitch(nodename string, runtime string) *batchv1.Job {
	...
	ttls := int32(100)
	job.GenerateName = "kubedig-snitch-"
	...
	job.Spec = batchv1.JobSpec{
		...
		Template: corev1.PodTemplateSpec{
			ObjectMeta: metav1.ObjectMeta{
				Labels: map[string]string{
					"kubedig-app": common.KubeDigSnitchRoleName,
				},
			},
			Spec: corev1.PodSpec{
				...
				// for some reasons github.com/zfz-725/KubeDig/KubeDig/utils/bpflsmprobe will
				...
				ServiceAccountName: common.KubeDigSnitchRoleName,
...
func (clusterWatcher *ClusterWatcher) deployControllerDeployment(deployment *appsv1.Deployment) error {
	...
	deployment.Spec.Template.Spec.Containers[0].Ports[0].ContainerPort = int32(common.KubeDigControllerPort)
	UpdateArgsIfDefinedAndUpdated(&deployment.Spec.Template.Spec.Containers[0].Args, []string{"webhook-port=" + strconv.Itoa(common.KubeDigControllerPort)})
...
func (clusterWatcher *ClusterWatcher) WatchRequiredResources() {
	...
	srvAccs := []*corev1.ServiceAccount{
		...
		addOwnership(deployments.GetKubeDigControllerServiceAccount(common.Namespace)).(*corev1.ServiceAccount),
	...
	controllerClusterRole := addOwnership(deployments.GetKubeDigControllerClusterRole()).(*rbacv1.ClusterRole)
	if annotateExisting {
	...
	clusterRoleBindings := []*rbacv1.ClusterRoleBinding{
		...
		addOwnership(deployments.GetKubeDigControllerClusterRoleBinding(common.Namespace)).(*rbacv1.ClusterRoleBinding),
		addOwnership(genSnitchRoleBinding()).(*rbacv1.ClusterRoleBinding),
	}
	roles := []*rbacv1.Role{
		addOwnership(deployments.GetKubeDigControllerLeaderElectionRole(common.Namespace)).(*rbacv1.Role),
	}
	roleBindings := []*rbacv1.RoleBinding{
		addOwnership(deployments.GetKubeDigControllerLeaderElectionRoleBinding(common.Namespace)).(*rbacv1.RoleBinding),
	}

	kubedigControllerWebhookSvc := deployments.GetKubeDigControllerWebhookService(common.Namespace)
	kubedigControllerWebhookSvc.Spec.Ports[0].TargetPort = intstr.FromInt(int(common.KubeDigControllerPort))
	svcs := []*corev1.Service{

		addOwnership(kubedigControllerWebhookSvc).(*corev1.Service),
	...
	// kubedig-controller and relay-server deployments
	controller := deployments.GetKubeDigControllerDeployment(common.Namespace)

	relayServer := deployments.GetRelayDeployment(common.Namespace)
	// update args, imagePullSecrets and tolerations
	UpdateArgsIfDefinedAndUpdated(&controller.Spec.Template.Spec.Containers[0].Args, common.KubeDigControllerArgs)
	...
	UpdateImagePullSecretsIfDefinedAndUpdated(&controller.Spec.Template.Spec.ImagePullSecrets, common.KubeDigControllerImagePullSecrets)
	UpdateTolerationsIfDefinedAndUpdated(&controller.Spec.Template.Spec.Tolerations, common.KubeDigControllerTolerations)
	...
	UpdateArgsIfDefinedAndUpdated(&relayServer.Spec.Template.Spec.Containers[0].Args, common.KubeDigRelayArgs)
	UpdateImagePullSecretsIfDefinedAndUpdated(&relayServer.Spec.Template.Spec.ImagePullSecrets, common.KubeDigControllerImagePullSecrets)
	UpdateTolerationsIfDefinedAndUpdated(&relayServer.Spec.Template.Spec.Tolerations, common.KubeDigControllerTolerations)
	...
	if common.EnableTls {
		relayServer.Spec.Template.Spec.Containers[0].VolumeMounts =
			append(relayServer.Spec.Template.Spec.Containers[0].VolumeMounts, common.KubeDigRelayTlsVolumeMount...)
		relayServer.Spec.Template.Spec.Volumes =
			append(relayServer.Spec.Template.Spec.Volumes, common.KubeDigRelayTlsVolume...)
	...
	relayServer.Spec.Template.Spec.Containers[0].Image = common.GetApplicationImage(common.KubeDigRelayName)
	relayServer.Spec.Template.Spec.Containers[0].ImagePullPolicy = corev1.PullPolicy(common.KubeDigRelayImagePullPolicy)
	deploys := []*appsv1.Deployment{
		addOwnership(relayServer).(*appsv1.Deployment),
	}

	// kubedig configmap
	configmap := addOwnership(deployments.GetKubedigConfigMap(common.Namespace, deployments.KubeDigConfigMapName)).(*corev1.ConfigMap)
	...
	for {
		caCert, tlsCrt, tlsKey, kGenErr = common.GeneratePki(common.Namespace, deployments.KubeDigControllerWebhookServiceName)
		if kGenErr == nil {
			break
		}
	...
	secret := deployments.GetKubeDigControllerTLSSecret(common.Namespace, caCert.String(), tlsCrt.String(), tlsKey.String())
	secret = addOwnership(secret).(*corev1.Secret)
	mutationhook := deployments.GetKubeDigControllerMutationAdmissionConfiguration(common.Namespace, caCert.Bytes())
...
func (clusterWatcher *ClusterWatcher) RotateTlsCerts() {
	...
	origdeploy, err := clusterWatcher.Client.AppsV1().Deployments(common.Namespace).Get(context.Background(), deployments.KubeDigControllerDeploymentName, metav1.GetOptions{})
	if err != nil {
		clusterWatcher.Log.Warnf("cannot get controller deployment, error=%s", err.Error())
	}

	caCert, tlsCrt, tlsKey, _ = common.GeneratePki(common.Namespace, deployments.KubeDigControllerWebhookServiceName)
	...
	controllerDeployment := deployments.GetKubeDigControllerDeployment(common.Namespace)
	...
	mutationWebhook := deployments.GetKubeDigControllerMutationAdmissionConfiguration(common.Namespace, caCert.Bytes())
	...
	controllerSecret := deployments.GetKubeDigControllerTLSSecret(common.Namespace, caCert.String(), tlsCrt.String(), tlsKey.String())
