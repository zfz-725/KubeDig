...
import (
	...
	controllers "github.com/zfz-725/KubeDig/pkg/KubeDigOperator/internal/controller"
	"github.com/zfz-725/KubeDig/pkg/KubeDigOperator/k8s"
...
var Cmd = &cobra.Command{
	...
	Use:   "kubedig-operator",
	Short: "An operator to install kubedig on k8s clusters",
	Long: `An operator to install kubedig on k8s clusters
	
KubeDig is a container-aware runtime security enforcement system that
...
func init() {
	...
	Cmd.PersistentFlags().StringVar(&DeploymentName, "deploymentName", "kubedig-operator", "operator deployment name")
	...
	Cmd.PersistentFlags().BoolVar(&AnnotateResource, "annotateResource", false, "when true kubedig annotate k8s resources with apparmor annotation")
	Cmd.PersistentFlags().BoolVar(&AnnotateExisting, "annotateExisting", false, "when true kubedig-controller restarts and annotates existing resources, with required annotations")
}
