...
// +kubebuilder:webhook:path=/mutate-pods,mutating=true,failurePolicy=Ignore,groups="",resources=pods;pods/binding,verbs=create;update,versions=v1,name=annotation.kubedig.com,admissionReviewVersions=v1,sideEffects=NoneOnDryRun
...
func (a *PodAnnotator) Handle(ctx context.Context, req admission.Request) admission.Response {
	...
	if req.Kind.Kind == "Binding" {
		...
		if _, exist := a.Cluster.Nodes[nodename]; exist {
			if a.Cluster.Nodes[nodename].KubeDigActive {
				annotate = true
			}
		}
