	...
	inf.AddEventHandler(cache.ResourceEventHandlerFuncs{
		AddFunc: func(obj interface{}) {
			if node, ok := obj.(*corev1.Node); ok {
				if node.Labels != nil {
					if enforcer, ok := node.Labels["kubedig.io/enforcer"]; ok {
						...
						if enforcer == "apparmor" {
							...
							kubedigStatus, err := common.CheckKubedigStatus(node.Name, c)
							if err != nil {
								log.Info(fmt.Sprintf("unable to get kubedig status on node %s : %s", node.Name, err.Error()))
							}
							cluster.Nodes[node.Name].KubeDigActive = kubedigStatus
							if !cluster.Nodes[node.Name].KubeDigActive {
								log.Info(fmt.Sprintf("kubedig not found on node %s :", node.Name))
							}
						}
			...
			if node, ok := newObj.(*corev1.Node); ok {
				...
				if enforcer, ok := node.Labels["kubedig.io/enforcer"]; ok {
					...
					if enforcer == "apparmor" {
						...
						kubedigStatus, err := common.CheckKubedigStatus(node.Name, c)
						if err != nil {
							log.Error(err, fmt.Sprintf("unable to get kubedig status on node %s", node.Name))
						}
						cluster.Nodes[node.Name].KubeDigActive = kubedigStatus
